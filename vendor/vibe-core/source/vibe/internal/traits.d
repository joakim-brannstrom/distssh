...
template checkInterfaceConformance(T, I) {
    ...
    import std.traits : FunctionAttribute, FunctionTypeOf, MemberFunctionsTuple,
        ParameterTypeTuple, ReturnType, functionAttributes;
                ...
                static if (functionAttributes!F & FunctionAttribute.property) {
                    static if (PT.length > 0) {
                        static if (!is(typeof(mixin(
                                "function RT (ref T t)" ~ attribs
                                ~ "{ return t." ~ mem ~ " = PT.init; }"))))
                            enum impl = T.stringof ~ " does not implement property setter \""
                                    ~ mem ~ "\" of type " ~ FT.stringof;
                        else
                                        enum string impl = impl!(i + 1);
                    } else {
                        static if (!is(typeof(mixin(
                                "function RT(ref T t)" ~ attribs ~ "{ return t." ~ mem ~ "; }"))))
                    ...
                    static if (is(RT == void)) {
                        static if (!is(typeof(mixin(
                                "function void(ref T t, ref PT p)" ~ attribs ~ "{ t."
                                ~ mem ~ "(p); }")))) {
                            static if (mem == "write" && PT.length == 2) {
                                auto f = mixin(
                                        "function void(ref T t, ref PT p)"
                                        ~ attribs ~ "{ t." ~ mem ~ "(p); }");
                            }
                        ...
                        static if (!is(typeof(mixin(
                                "function RT(ref T t, ref PT p)" ~ attribs
                                ~ "{ return t." ~ mem ~ "(p); }"))))
                            enum impl = T.stringof ~ " does not implement method \""
                                    ~ mem ~ "\" of type " ~ FT.stringof;
                        else
...
unittest {
    ...
    static assert(checkInterfaceConformance!(NonOSStruct,
    ...
    static assert(checkInterfaceConformance!(NonOSStruct2,
            OutputStream) == "NonOSStruct2 does not implement method \"write\" of type @safe void(const(ubyte[]) bytes)");
}
