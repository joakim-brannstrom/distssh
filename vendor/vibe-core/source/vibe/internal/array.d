...
struct AllocAppender(ArrayType : E[], E) {
    ...
    this(IAllocator alloc, ElemType[] initial_buffer = null) @safe {
        m_alloc = alloc;
	...
	*/
    void reserve(size_t amount) @safe {
        ...
        if (!m_data.length) {
            m_data = () @trusted{
                return cast(ElemType[]) m_alloc.allocate(amount * E.sizeof);
            }();
            m_remaining = m_data;
        ...
        if (m_remaining.length < amount) {
            ...
            if (m_allocatedBuffer)
                ...
                auto newdata = () @trusted{
                    return cast(ElemType[]) m_alloc.allocate((nelems + amount) * E.sizeof);
                }();
...
struct FixedRingBuffer(T, size_t N = 0, bool INITIALIZE = true) {
    ...
    static if (N == 0) {
        ...
        ~this() {
            if (m_freeOnDestruct && m_buffer.length > 0)
    ...
    static if (N == 0) {
        deprecated @property void freeOnDestruct(bool b) {
            m_freeOnDestruct = b;
        }
    ...
    void removeAt(Range r) {
        ...
        if (m_start + m_fill > m_buffer.length) {
            assert(r.m_start >= m_start && r.m_start < m_buffer.length
                    || r.m_start < mod(m_start + m_fill));
...
struct BatchBuffer(T, size_t N = 0) {
    ...
    static if (N == 0) {
        @property void capacity(size_t n) {
            assert(n >= m_fill);
            m_buffer.length = n;
        }
    }
    ...
    @property bool empty() const {
        assert(m_first < m_fill || m_fill == 0 && m_first == 0);
        return m_first >= m_fill;
    }
    ...
    inout(T)[] peek() inout {
        return m_buffer[m_first .. m_fill];
    }
    ...
    void putN(size_t n) {
        assert(empty && n <= m_buffer.length);
        m_fill = n;
    }

    void putN(T[] elems) {
        assert(empty && elems.length <= m_buffer.length);
        m_buffer[0 .. elems.length] = elems[];
        m_fill = elems.length;
    }
