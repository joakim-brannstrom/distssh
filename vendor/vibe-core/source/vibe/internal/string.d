...
*/
string sanitizeUTF8(in ubyte[] str) @safe pure {
    import std.utf;
    ...
    while (i < str.length) {
        dchar ch = str[i];
        try
            ch = std.utf.decode(cast(const(char[])) str, i);
        catch (UTFException) {
            i++;
        }
        //catch( AssertError ){ i++; }
...
*/
bool allOf(string str, string chars) @safe pure {
...
ptrdiff_t indexOfCT(Char)(in Char[] s, dchar c, CaseSensitive cs = CaseSensitive.yes) @safe pure {
    if (__ctfe) {
...
ptrdiff_t indexOfCT(Char)(in Char[] s, in Char[] needle) {
    if (__ctfe) {
        ...
        return std.string.indexOf(s, needle);
}

/**
	Checks if any character in 'str' is contained in 'chars'.
*/
bool anyOf(string str, string chars) @safe pure {
    foreach (ch; str)
        if (chars.canFind(ch))
            return true;
    return false;
...
string stripLeftA(string s) @safe pure nothrow {
...
/// ASCII whitespace trimming (space and tab)
string stripRightA(string s) @safe pure nothrow {
...
/// ASCII whitespace trimming (space and tab)
string stripA(string s) @safe pure nothrow {
    return stripLeftA(stripRightA(s));
}

/// Finds the first occurence of any of the characters in `chars`
sizediff_t indexOfAny(string str, string chars) @safe pure {
...
*/
sizediff_t matchBracket(string str, bool nested = true) @safe pure nothrow {
    if (str.length < 2)
        return -1;
...
@safe unittest {
    ...
    enum tests = [Test("[foo]", 4), Test("<bar>", 4), Test("{baz}", 4),
...
int icmp2(string a, string b) @safe pure {
    size_t i = 0, j = 0;
    ...
    while (i < min_len && a[i] == b[i])
        i++;
    if (i > 0 && (a[i - 1] & 0x80))
        i--; // don't stop half-way in a UTF-8 sequence
    j = i;
