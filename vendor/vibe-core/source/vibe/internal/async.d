...
auto asyncAwait(Callback, alias action, alias cancel)(string func = __FUNCTION__)
    ...
    alias waitable = Waitable!(Callback, action, cancel, (ParameterTypeTuple!Callback r) {
        results = r;
    });
...
auto asyncAwait(Callback, alias action, alias cancel)(Duration timeout, string func = __FUNCTION__) {
    ...
    static if (is(ReturnType!action == void)) {
        alias waitable = Waitable!(Callback, action, (cb) {
            ret.completed = false;
            cancel(cb);
        }, (ParameterTypeTuple!Callback r) { ret.results = r; });
    } else {
        alias waitable = Waitable!(Callback, action, (cb, waitres) {
            ret.completed = false;
            cancel(cb, waitres);
        }, (ParameterTypeTuple!Callback r) { ret.results = r; });
    }
...
auto asyncAwaitUninterruptible(Callback, alias action)(string func = __FUNCTION__) nothrow {
    static if (is(typeof(action(Callback.init)) == void))
        void cancel(Callback) {
            assert(false, "Action cannot be cancelled.");
        }
    else
        void cancel(Callback, typeof(action(Callback.init))) @safe @nogc nothrow {
            assert(false, "Action cannot be cancelled.");
        }
...
auto asyncAwaitUninterruptible(Callback, alias action, alias cancel)(
        Duration timeout, string func = __FUNCTION__) nothrow {
    ParameterTypeTuple!Callback results;
    alias waitable = Waitable!(Callback, action, cancel, (ParameterTypeTuple!Callback r) {
        results = r;
    });
...
template Waitable(CB, alias WAIT, alias CANCEL, alias DONE) {
    ...
    static assert(is(typeof(WAIT(CB.init))),
            "WAIT must be callable with a parameter of type " ~ CB.stringof);
    static if (is(typeof(WAIT(CB.init)) == void))
        ...
        static assert(is(typeof(CANCEL(CB.init, typeof(WAIT(CB.init)).init))),
                "CANCEL must be callable with parameters " ~ CB.stringof ~ " and " ~ typeof(
                    WAIT(CB.init)).stringof);
...
void asyncAwaitAny(bool interruptible, Waitables...)(Duration timeout, string func = __FUNCTION__) {
    if (timeout == Duration.max)
        ...
        alias timerwaitable = Waitable!(TimerCallback, cb => eventDriver.timers.wait(tm,
                cb), cb => eventDriver.timers.cancelWait(tm), (tid) {  });
...
void asyncAwaitAny(bool interruptible, Waitables...)(string func = __FUNCTION__)
    ...
    static string waitableCode() {
        ...
        foreach (i, W; Waitables) {
			...
			}.format(i, generateParamDecls!(CBDel!W)(format("PT%s",
                    i)), generateParamNames!(CBDel!W));
        }
        return ret;
    }
    ...
    debug (VibeAsyncLog)
        logDebugV("Need to wait in %s (%s)...", func, interruptible
    ...
    debug (VibeAsyncLog)
        scope (failure)
            logDebugV("Aborting wait due to exception");
...
@safe nothrow  /*@nogc*/ unittest {
    int a, b, c;
    int w1r, w2r;
    alias w1 = Waitable!(void delegate(int) @safe nothrow, (cb) { a++; cb(42); }, (cb) {
        assert(false);
    }, (i) { w1r = i; });
    alias w2 = Waitable!(void delegate(int) @safe nothrow, (cb) { b++; }, (cb) {
        c++;
    }, (i) { w2r = i; });
    alias w3 = Waitable!(void delegate(int) @safe nothrow, (cb) { c++; cb(42); }, (cb) {
        assert(false);
    }, (int n) { assert(n == 42); });
...
private string generateParamDecls(Fun)(string ptypes_name = "PTypes") {
    ...
    foreach (i, T; Types) {
        static if (i > 0)
            ret ~= ", ";
        static if (SClasses[i] & ParameterStorageClass.lazy_)
            ret ~= "lazy ";
        static if (SClasses[i] & ParameterStorageClass.scope_)
            ret ~= "scope ";
        static if (SClasses[i] & ParameterStorageClass.out_)
            ret ~= "out ";
        static if (SClasses[i] & ParameterStorageClass.ref_)
            ret ~= "ref ";
...
private template hasAnyScopeParameter(Callback) {
    ...
    static if (SC.length == 0)
        enum hasAnyScopeParameter = false;
    else
        enum hasAnyScopeParameter = any!(c => c & ParameterStorageClass.scope_)([SC]);
}
