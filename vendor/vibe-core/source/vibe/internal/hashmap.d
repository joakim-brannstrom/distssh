...
struct DefaultHashMapTraits(Key) {
    ...
    static size_t hashOf(in ref Key k) {
        static if (is(Key == class) && &Key.toHash == &Object.toHash)
            ...
            static @nogc nothrow size_t properlyTypedWrapper(in ref Key k) {
                return 0;
            }
...
@safe:
    ...
    struct TableEntry {
        ...
        this(Key key, Value value) @trusted {
    ...
    ~this() {
        ...
        if (m_table.ptr !is null)
            () @trusted{
            static if (hasIndirections!TableEntry)
                GC.removeRange(m_table.ptr);
            try
                m_allocator.dispose(m_table);
            catch (Exception e)
                assert(false, e.msg);
        }();
    }
    ...
    void opIndexAssign(Value value, Key key) {
        ...
        if (!Traits.equals(m_table[i].key, key))
            m_length++;
            ...
            if (!Traits.equals(m_table[i].key, Traits.clearValue)) {
                ...
                static if (arity!del == 1) {
                    if (int ret = del(m_table[i].value))
                        return ret;
                } else if (int ret = del(m_table[i].key, m_table[i].value))
                    return ret;
            }
        return 0;
    }
    ...
    private size_t findInsertIndex(Key key) const {
        ...
        while (!Traits.equals(m_table[i].key, Traits.clearValue)
                && !Traits.equals(m_table[i].key, key)) {
            if (++i >= m_table.length)
                i -= m_table.length;
    ...
    private void grow(size_t amount) {
        ...
        while (newsize >= (newcap * 2) / 3)
            newcap *= 2;
        resize(newcap);
    }

    private void resize(size_t new_size) @trusted nothrow {
        assert(!m_resizing);
        m_resizing = true;
        scope (exit)
            m_resizing = false;
        ...
        if (!m_allocator) {
            try
                m_allocator = vibeThreadAllocator();
            catch (Exception e)
                assert(false, e.msg);
        }
        ...
        try
            ...
            assert(false, e.msg);
        static if (hasIndirections!TableEntry)
            GC.addRange(m_table.ptr, m_table.length * TableEntry.sizeof);
        ...
        if (oldtable !is null) {
            static if (hasIndirections!TableEntry)
                GC.removeRange(oldtable.ptr);
            try
                m_allocator.deallocate(oldtable);
            catch (Exception e)
                assert(false, e.msg);
        }
    }
