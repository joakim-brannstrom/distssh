...
struct FreeListObjectAlloc(T, bool USE_GC = true, bool INIT = true, EXTRA = void) {
    ...
    static TR alloc(ARGS...)(ARGS args) {
        ...
        static if (INIT)
            internalEmplace!(Unqual!T)(mem, args);
    ...
    static void free(TR obj) {
        static if (INIT) {
            scope (failure)
                assert(0, "You shouldn't throw in destructors");
            auto objc = obj;
            static if (is(TR == T*))
...
struct FreeListRef(T, bool INIT = true) {
    ...
    static if (is(T == class)) {
        @property inout(T) get() inout @safe nothrow {
            return m_object;
        }
    } else {
        @property ref inout(T) get() inout @safe nothrow {
            return *m_object;
        }
    }
    alias get this;

    private @property ref int refCount() const @trusted {
        auto ptr = cast(ubyte*) cast(void*) m_object;
        ptr += ElemSize;
        return *cast(int*) ptr;
    }

    private void checkInvariants() const {
...
in {
    ...
    assert(chunk.length >= T.sizeof, format("emplace: Chunk size too small: %s < %s size = %s",
...
body {
    ...
    static if (is(typeof(result.__ctor(args)))) {
        ...
        static assert(args.length == 0 && !is(typeof(&T.__ctor)),
                "Don't know how to initialize an object of type " ~ T.stringof
                ~ " with arguments " ~ Args.stringof);
    }
    return result;
}
