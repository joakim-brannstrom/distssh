...
/**
	...
	See_also: ` vibe.core.args.finalizeCommandLineOptions`, `lowerPrivileges`,
		`runEventLoop`
*/
int runApplication(string[]* args_out = null) @safe {
    try
        if (!()@trusted{ return finalizeCommandLineOptions(); }())
            return 0;
    catch (Exception e) {
...
int runEventLoop() @safe nothrow {
    ...
    scope (exit) {
        ...
        () @trusted nothrow{
            scope (failure)
                assert(false); // notifyAll is not marked nothrow
...
*/
void exitEventLoop(bool shutdown_all_threads = false) @safe nothrow {
...
*/
bool processEvents() @safe nothrow {
...
*/
ExitReason runEventLoopOnce() @safe nothrow {
...
*/
void setIdleHandler(void delegate() @safe nothrow del) @safe nothrow {
    s_idleHandler = () @safe nothrow{ del(); return false; };
}
/// ditto
void setIdleHandler(bool delegate() @safe nothrow del) @safe nothrow {
    s_idleHandler = del;
}
...
///
Task runTask(ARGS...)(void delegate(ARGS) @system task, auto ref ARGS args) @system {
    return runTask_internal!((ref tfi) { tfi.set(task, args); });
}
/// ditto
...
package Task runTask_internal(alias TFI_SETUP)() {
    ...
    while (!f && !s_availableFibers.empty) {
        ...
        if (()@trusted nothrow{ return f.state; }() != Fiber.State.HOLD)
            f = null;
    }

    if (f is null) {
        // if there is no fiber available, create one.
        if (s_availableFibers.capacity == 0)
            s_availableFibers.capacity = 1024;
    ...
    debug if (TaskFiber.ms_taskCreationCallback) {
        ...
        info.handle = handle;
        info.functionPointer = () @trusted{
            return cast(void*) f.m_taskFunc.functionPointer;
        }();
    ...
    s_scheduler.switchTo(handle, TaskFiber.getThis().m_yieldLockCount > 0
            ? Flag!"defer".yes : Flag!"defer".no);

    debug if (TaskFiber.ms_taskEventCallback) {
        () @trusted{
            TaskFiber.ms_taskEventCallback(TaskEvent.postStart, handle);
        }();
    }
...
unittest {
    static class Test {
        void workerMethod(int param) shared {
            logInfo("Param: %s", param);
        }
    }
...
unittest {
    ...
    static void test719() {
        runWorkerTaskH(&work719, cast(ubyte) 42);
    }
}
...
void yield() @safe {
    ...
    if (t != Task.init) {
        ...
        assert(TaskFiber.getThis().m_yieldLockCount == 0,
                "May not yield within an active yieldLock()!");
...
*/
void hibernate(scope void delegate() @safe nothrow on_interrupt = null) @safe nothrow {
    auto t = Task.getThis();
    if (t == Task.init) {
        assert(TaskFiber.getThis().m_yieldLockCount == 0,
                "May not yield within an active yieldLock!");
...
/**
	...
	See_Also: `hibernate`
*/
void switchToTask(Task t) @safe nothrow {
...
/**
	...
	Throws: May throw an `InterruptException` if the task gets interrupted using
		`Task.interrupt()`.
*/
void sleep(Duration timeout) @safe {
...
Timer setTimer(Duration timeout, void delegate() callback, bool periodic = false) @system nothrow {
...
/**
	...
	See_also: setTimer
*/
Timer createTimer(void delegate() nothrow @safe callback) @safe nothrow {
...
/**
	...
	Returns:
		...
		file descriptor.
*/
FileDescriptorEvent createFileDescriptorEvent(int file_descriptor,
        FileDescriptorEvent.Trigger event_mask) @safe nothrow {
...
*/
void setTaskStackSize(size_t sz) nothrow {
...
out (count) {
    assert(count > 0, "No worker threads started after setupWorkerThreads!?");
}
body {
    setupWorkerThreads();
    synchronized (st_threadsMutex)
...
void lowerPrivileges(string uname, string gname) @safe {
    ...
    if (uname != "" || gname != "") {
        ...
        int uid = -1, gid = -1;
        if (uname != "" && !tryParse(uname, uid))
            uid = getUID(uname);
        if (gname != "" && !tryParse(gname, gid))
            gid = getGID(gname);
        setUID(uid, gid);
    } else
...
@safe:
    ...
    private this(int fd, Trigger event_mask) nothrow {
	...
	*/
    ...
    bool wait(Duration timeout, Trigger which = Trigger.any) {
        if ((which & m_context.trigger) == Trigger.none)
            return true;

        assert((which & m_context.trigger) == Trigger.read,
                "Waiting for write event not yet supported.");

        bool got_data;

        alias readwaiter = Waitable!(IOCallback, cb => eventDriver.sockets.waitForData(m_socket,
                cb), cb => eventDriver.sockets.cancelRead(m_socket), (fd, st, nb) {
            got_data = st == IOStatus.ok;
        });
...
@safe:
    ...
    private this(TimerID id) nothrow {
    ...
    this(this) nothrow {
        ...
        if (m_driver)
            m_driver.timers.addRef(m_id);
    }

    ~this() nothrow {
        debug assert(m_magicNumber == 0x4d34f916, "Timer corrupted.");
        if (m_driver)
            releaseHandle!"timers"(m_id, () @trusted{
    ...
    @property bool pending() nothrow {
        return m_driver.timers.isPending(m_id);
    }

    /// The internal ID of the timer.
    @property size_t id() const nothrow {
        return m_id;
    }

    bool opCast() const nothrow {
        return m_driver !is null;
    }

    /// Determines if this reference is the only one
    @property bool unique() const nothrow {
        return m_driver ? m_driver.timers.isUnique(m_id) : false;
    }
	...
	*/
    ...
    in {
        assert(dur > 0.seconds, "Negative timer duration specified.");
    }
    body {
        m_driver.timers.set(m_id, dur, periodic ? dur : 0.seconds);
    }
	...
	*/
    void stop() nothrow {
        if (m_driver)
            m_driver.timers.stop(m_id);
    }

    /** Waits until the timer fires.
	*/
    void wait() {
        asyncAwait!(TimerCallback, cb => m_driver.timers.wait(m_id, cb),
...
package(vibe) void performIdleProcessing() @safe nothrow {
    ...
    while (again) {
        ...
        if (again) {
            ...
            if (er.among!(ExitReason.exited, ExitReason.outOfWaiters)
                    && s_scheduler.scheduledTaskCount == 0) {
    ...
    if (s_scheduler.scheduledTaskCount)
        logDebug("Exiting from idle processing although there are still yielded tasks");

    if (!s_ignoreIdleForGC && s_gcTimer) {
        s_gcTimer.rearm(s_gcCollectTimeout);
    } else
        s_ignoreIdleForGC = false;
}
...
private bool getExitFlag() @trusted nothrow {
    return s_exitEventLoop || atomicLoad(st_term);
}

package @property bool isEventLoopRunning() @safe nothrow @nogc {
    return s_eventLoopRunning;
}

package @property ref TaskScheduler taskScheduler() @safe nothrow @nogc {
    return s_scheduler;
}

package void recycleFiber(TaskFiber fiber) @safe nothrow {
    if (s_availableFibers.length >= s_maxRecycledFibers) {
        ...
        () @trusted{
            try
                ...
                logWarn("Failed to destroy fiber: %s", e.msg);
        }();
    }
...
private void setupSignalHandlers() @trusted nothrow {
    scope (failure)
        assert(false); // _d_monitorexit is not nothrow
...
shared static this() {
    ...
    version (VibeNoDefaultArgs) {
        ...
        readOption("uid|user", &s_privilegeLoweringUserName,
                "Sets the user name or id used for privilege lowering.");
        readOption("gid|group", &s_privilegeLoweringGroupName,
                "Sets the group name or id used for privilege lowering.");
    }
...
static this() {
    ...
    if (Thread.getThis().isDaemon && Thread.getThis().name == "CmdProcessor")
        return;
...
private void shutdownWorkerPool() nothrow {
    shared(TaskPool) tpool;

    try
        synchronized (st_threadsMutex)
            swap(tpool, st_workerPool);
    catch (Exception e)
        assert(false, e.msg);
...
private extern (C) void extrap() @safe nothrow {
    logTrace("exception trap");
}

private extern (C) void onSignal(int signal) nothrow {
...
private extern (C) void onBrokenPipe(int signal) nothrow {
...
version (Posix) {
    ...
    private void setUID(int uid, int gid) @trusted {
