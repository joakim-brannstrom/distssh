...
import vibe.core.core : exitEventLoop, logicalProcessorCount, runEventLoop,
    runTask, runTask_internal;
import vibe.core.log;
import vibe.core.sync : ManualEvent, Monitor, SpinLock, createSharedManualEvent,
    createMonitor;
...
shared final class TaskPool {
	...
	*/
    this(size_t thread_count = logicalProcessorCount()) @safe {
	...
	*/
    @property size_t threadCount() const shared {
	...
	*/
    void runTask(FT, ARGS...)(FT func, auto ref ARGS args) if (isFunctionPointer!FT) {
        foreach (T; ARGS)
            static assert(isWeaklyIsolated!T,
                    "Argument type " ~ T.stringof ~ " is not safe to pass between threads.");
	...
	*/
    Task runTaskH(FT, ARGS...)(FT func, auto ref ARGS args)
        ...
        assert(caller != Task.init,
                "runWorkderTaskH can currently only be called from within a task.");
	...
	*/
    ...
    private void runTask_unsafe(CALLABLE, ARGS...)(CALLABLE callable, ref ARGS args) {
        ...
        static assert(areConvertibleTo!(Group!ARGS, Group!FARGS),
                "Cannot convert arguments '" ~ ARGS.stringof
                ~ "' to function arguments '" ~ FARGS.stringof ~ "'.");
...
private final class WorkerThread : Thread {
    ...
    private void main() nothrow {
        import core.stdc.stdlib : abort;
        import core.exception : InvalidMemoryOperationError;
        import std.encoding : sanitize;

        try {
            if (m_pool.m_state.lock.term)
                return;
    ...
    private void handleWorkerTasks() nothrow @safe {
        ...
        while (true) {
            ...
            if (taskfunc.func !is null)
                ...
                emit_count = m_pool.m_signal.waitUninterruptible(emit_count);
        }
...
nothrow @safe:
    ...
    void put(CALLABLE, ARGS...)(ref CALLABLE c, ref ARGS args) {
        ...
        if (m_queue.full)
            m_queue.capacity = max(16, m_queue.capacity * 3 / 2);
        assert(!m_queue.full);
