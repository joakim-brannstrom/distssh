...
*/
NetworkAddress resolveHost(string host,
        AddressFamily address_family = AddressFamily.UNSPEC, bool use_dns = true) {
...
NetworkAddress resolveHost(string host, ushort address_family, bool use_dns = true) {
    ...
    version (Windows)
        import core.sys.windows.winsock2 : sockaddr_in, sockaddr_in6;
    else
    ...
    if (host[0] == ':' || host[0] >= '0' && host[0] <= '9') {
        ...
        case INET:
            *ret.sockAddrInet4 = () @trusted{
                return *cast(sockaddr_in*) addr.name;
            }();
            break;
        case INET6:
            *ret.sockAddrInet6 = () @trusted{
                return *cast(sockaddr_in6*) addr.name;
            }();
            break;
        }
        return ret;
        ...
        NetworkAddress res;
        bool success = false;
        alias waitable = Waitable!(DNSLookupCallback, cb => eventDriver.dns.lookupHost(host,
                cb), (cb, id) => eventDriver.dns.cancelLookup(id), (DNSLookupID,
                DNSStatus status, scope RefAddress[] addrs) {
            if (status == DNSStatus.ok && addrs.length > 0) {
                try
                    res = NetworkAddress(addrs[0]);
                catch (Exception e) {
                    logDiagnostic("Failed to store address from DNS lookup: %s", e.msg);
                }
                success = true;
            }
        });
...
*/
TCPListener[] listenTCP(ushort port, TCPConnectionDelegate connection_callback,
        TCPListenOptions options = TCPListenOptions.defaults) {
    TCPListener[] ret;
    try
        ret ~= listenTCP(port, connection_callback, "::", options);
    catch (Exception e)
        logDiagnostic("Failed to listen on \"::\": %s", e.msg);
    try
        ret ~= listenTCP(port, connection_callback, "0.0.0.0", options);
    catch (Exception e)
        logDiagnostic("Failed to listen on \"0.0.0.0\": %s", e.msg);
    enforce(ret.length > 0, format("Failed to listen on all interfaces on port %s", port));
    return ret;
}
/// ditto
TCPListener listenTCP(ushort port, TCPConnectionDelegate connection_callback,
        string address, TCPListenOptions options = TCPListenOptions.defaults) {
...
deprecated("Use a @safe nothrow callback instead.") TCPListener[] listenTCP(ushort port,
...
/// ditto
deprecated("Use a @safe nothrow callback instead.") TCPListener listenTCP(ushort port,
        void delegate(TCPConnection) connection_callback, string address,
        TCPListenOptions options = TCPListenOptions.defaults) {
...
TCPConnection connectTCP(string host, ushort port, string bind_interface = null,
    ...
    if (bind_interface.length)
        bind_address = resolveHost(bind_interface, addr.family);
    else {
        bind_address.family = addr.family;
        if (bind_address.family == AddressFamily.INET)
            bind_address.sockAddrInet4.sin_addr.s_addr = 0;
        else if (bind_address.family != AddressFamily.UNIX)
            bind_address.sockAddrInet6.sin6_addr.s6_addr[] = 0;
    }
    if (addr.family != AddressFamily.UNIX)
...
TCPConnection connectTCP(NetworkAddress addr,
        NetworkAddress bind_address = anyAddress, Duration timeout = Duration.max) {
    import std.conv : to;
    ...
    enforce(addr.family == bind_address.family,
            "Destination address and bind address have different address families.");
    ...
    return () @trusted{ // scope
        ...
        alias waiter = Waitable!(ConnectCallback,
                cb => eventDriver.sockets.connectStream(uaddr, baddr, cb), (cb, sock_fd) {
                    cancelled = true;
        ...
        enforce(!cancelled, "Failed to connect to " ~ addr.toString() ~ ": timeout");
        enforce(status == ConnectStatus.connected,
                "Failed to connect to " ~ addr.toString() ~ ": " ~ status.to!string);
...
@safe:
	...
	*/
    @property ushort family() const pure nothrow {
        return addr.sa_family;
    }
    /// ditto
    @property void family(AddressFamily val) pure nothrow {
        addr.sa_family = cast(ubyte) val;
    }
    /// ditto
    @property void family(ushort val) pure nothrow {
        addr.sa_family = cast(ubyte) val;
    }
	...
	*/
    @property ushort port() const pure nothrow {
        ...
        switch (this.family) {
        default:
            assert(false, "port() called for invalid address family.");
        case AF_INET:
            nport = addr_ip4.sin_port;
            break;
        case AF_INET6:
            nport = addr_ip6.sin6_port;
            break;
        }
        return () @trusted{ return ntoh(nport); }();
    }
    /// ditto
    @property void port(ushort val) pure nothrow {
        auto nport = () @trusted{ return hton(val); }();
        switch (this.family) {
        default:
            assert(false, "port() called for invalid address family.");
        case AF_INET:
            addr_ip4.sin_port = nport;
            break;
        case AF_INET6:
            addr_ip6.sin6_port = nport;
            break;
        }
    }
	...
	*/
    @property inout(sockaddr)* sockAddr() inout pure nothrow {
        return &addr;
    }

    /** Size of the sockaddr struct that is returned by sockAddr().
	*/
    @property socklen_t sockAddrLen() const pure nothrow {
        switch (this.family) {
        default:
    ...
    version (Posix) {
        ...
        in {
            assert(family == AddressFamily.UNIX);
        }
        body {
            return &addr_unix;
        }
	...
	*/
    string toAddressString() const nothrow {
    ...
    /// ditto
    void toAddressString(scope void delegate(const(char)[]) @safe sink) const nothrow {
        import std.array : appender;
        ...
        switch (this.family) {
        default:
            assert(false, "toAddressString() called for invalid address family.");
        case AF_UNSPEC:
            sink("<UNSPEC>");
            break;
        case AF_INET: {
                ubyte[4] ip = () @trusted{
                    return (cast(ubyte*)&addr_ip4.sin_addr.s_addr)[0 .. 4];
                }();
        ...
        case AddressFamily.UNIX:
                ...
                static if (hasMember!(sockaddr_un, "sun_len"))
                    sink(() @trusted{
                        return cast(char[]) addr_unix.sun_path[0 .. addr_unix.sun_len];
                    }());
                else
                    sink(() @trusted{
                        return (cast(char*) addr_unix.sun_path.ptr).fromStringz;
                    }());
                break;
            }
        }
	...
	*/
    ...
    /// ditto
    void toString(scope void delegate(const(char)[]) @safe sink) const nothrow {
        import std.format : formattedWrite;

        try {
            switch (this.family) {
            default:
                assert(false, "toString() called for invalid address family.");
            case AF_UNSPEC:
                sink("<UNSPEC>");
                break;
            case AF_INET:
    ...
    version (Have_libev) {
        ...
        unittest {
            void test(string ip) {
                ...
                assert(res == ip, "IP " ~ ip ~ " yielded wrong string representation: " ~ res);
            }
...
@safe:
    ...
    private this(StreamSocketFD socket, scope RefAddress remote_address) nothrow {
        import std.exception : enforce;

        m_socket = socket;
        m_context = () @trusted{
            return &eventDriver.sockets.userData!Context(socket);
        }();
    ...
    bool opCast(T)() const nothrow if (is(T == bool)) {
        return m_socket != StreamSocketFD.invalid;
    }

    @property void tcpNoDelay(bool enabled) nothrow {
        eventDriver.sockets.setTCPNoDelay(m_socket, enabled);
        m_context.tcpNoDelay = enabled;
    }

    @property bool tcpNoDelay() const nothrow {
        return m_context.tcpNoDelay;
    }

    @property void keepAlive(bool enabled) nothrow {
        eventDriver.sockets.setKeepAlive(m_socket, enabled);
        m_context.keepAlive = enabled;
    }

    @property bool keepAlive() const nothrow {
        return m_context.keepAlive;
    }

    @property void readTimeout(Duration duration) {
        m_context.readTimeout = duration;
    }

    @property Duration readTimeout() const nothrow {
        return m_context.readTimeout;
    }

    @property string peerAddress() const nothrow {
    ...
    bool waitForData(Duration timeout = Duration.max) {
        ...
        alias waiter = Waitable!(IOCallback, cb => eventDriver.sockets.read(m_socket,
                m_context.readBuffer.peekDst(), mode, cb), (cb) {
            cancelled = true;
	...
	*/
    WaitForDataAsyncStatus waitForDataAsync(CALLABLE)(CALLABLE read_ready_callback,
    ...
    const(ubyte)[] peek() {
        return m_context ? m_context.readBuffer.peek() : null;
    }
    ...
    size_t write(in ubyte[] bytes, IOMode mode) {
        ...
        auto res = asyncAwait!(IOCallback, cb => eventDriver.sockets.write(m_socket,
                bytes, mode, cb), cb => eventDriver.sockets.cancelWrite(m_socket));
    ...
    private void writeDefault(InputStream)(InputStream stream, ulong nbytes = 0)
        ...
        scope buffer = () @trusted{
            return cast(ubyte[]) theAllocator.allocate(64 * 1024);
        }();
        scope (exit)
            () @trusted{ theAllocator.dispose(buffer); }();
...
struct TCPListener {
    ...
    this(StreamListenSocketFD socket) {
        m_socket = socket;
        m_context = () @trusted{
            return &eventDriver.sockets.userData!Context(m_socket);
        }();
        m_context.driver = () @trusted{ return cast(shared) eventDriver; }();
    }

    bool opCast(T)() const nothrow if (is(T == bool)) {
        return m_socket != StreamListenSocketFD.invalid;
    }
...
struct UDPConnection {
    ...
    bool opCast(T)() const nothrow if (is(T == bool)) {
        return m_socket != DatagramSocketFD.invalid;
    }
	...
	*/
    @property bool canBroadcast() const {
        return m_context.canBroadcast;
    }
    /// ditto
    @property void canBroadcast(bool val) {
        enforce(eventDriver.sockets.setBroadcast(m_socket, val),
                "Failed to set UDP broadcast flag.");
        m_context.canBroadcast = val;
    }
    ...
    @property NetworkAddress localAddress() const nothrow {
        ...
        scope addr = new RefAddress(naddr.sockAddr, naddr.sockAddrMaxLen);
        try {
            enforce(eventDriver.sockets.getLocalAddress(m_socket, addr),
                    "Failed to query socket address.");
        }
        catch (Exception e) {
            logWarn("Failed to get local address for TCP connection: %s", e.msg);
        }
        return naddr;
    }
	...
	*/
    @property void multicastLoopback(bool loop) {
        enforce(eventDriver.sockets.setOption(m_socket, DatagramSocketOption.multicastLoopback,
                loop), "Failed to set multicast loopback mode.");
    }
	...
	*/
    void addMembership(ref NetworkAddress multiaddr, uint interface_index = 0) {
        ...
        enforce(eventDriver.sockets.joinMulticastGroup(m_socket, addr,
                interface_index), "Failed to add multicast membership.");
    }
	...
	*/
    void send(in ubyte[] data, in NetworkAddress* peer_address = null) {
        ...
        if (peer_address)
            addrc.set(() @trusted{
                return (cast(NetworkAddress*) peer_address).sockAddr;
            }(), peer_address.sockAddrLen);

        IOStatus status;
        size_t nbytes;
        bool cancelled;

        alias waitable = Waitable!(DatagramIOCallback, cb => eventDriver.sockets.send(m_socket,
                data, IOMode.once, peer_address ? addrc : null, cb), (cb) {
            cancelled = true;
            eventDriver.sockets.cancelSend(m_socket);
	...
	*/
    ...
    ubyte[] recv(Duration timeout, ubyte[] buf = null, NetworkAddress* peer_address = null) {
        ...
        alias waitable = Waitable!(DatagramIOCallback,
                cb => eventDriver.sockets.receive(m_socket, buf, IOMode.once, cb), (cb) {
                    cancelled = true;
                    eventDriver.sockets.cancelReceive(m_socket);
                }, (DatagramSocketFD, IOStatus status_, size_t nbytes_, scope RefAddress addr) {
                    status = status_;
                    nbytes = nbytes_;
                    if (status_ == IOStatus.ok && peer_address) {
                        try
                            *peer_address = NetworkAddress(addr);
                        catch (Exception e)
                            logWarn("Failed to store datagram source address: %s", e.msg);
                    }
                });
...
private pure nothrow {
    ...
    ushort ntoh(ushort val) {
        version (LittleEndian)
            ...
            static assert(false, "Unknown endianness.");
    }
