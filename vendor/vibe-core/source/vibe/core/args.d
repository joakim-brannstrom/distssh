...
bool readOption(T)(string names, T* pvalue, string help_text)
    ...
    assert(!g_options.any!(o => o.names == info.names)(),
            "readOption() may only be called once per option name.");
    g_options ~= info;

    immutable olen = g_args.length;
    getopt(g_args, getoptConfig, names, pvalue);
    if (g_args.length < olen)
        return true;
...
unittest {
    ...
    int i;
    readOption("c", &i, "");
    assert(i == 2_000_000_000);
    long l;
    readOption("d", &l, "");
    assert(l == 8_000_000_000);
    float f;
    readOption("e", &f, "");
    assert(f == cast(float) 1.0);
    double d;
    readOption("f", &d, "");
    assert(d == 2.0);
    string st;
    ...
    bool[] ba;
    readOption("h", &ba, "");
    assert(ba == [false, true]);
    short[] sa;
    readOption("i", &sa, "");
    assert(sa == [-16000, 16000]);
    int[] ia;
    readOption("j", &ia, "");
    assert(ia == [-2_000_000_000, 2_000_000_000]);
    long[] la;
    readOption("k", &la, "");
    assert(la == [-8_000_000_000, 8_000_000_000]);
    float[] fa;
    readOption("l", &fa, "");
    assert(fa == [cast(float)-1.0, cast(float) 1.0]);
    double[] da;
    readOption("m", &da, "");
    assert(da == [-2.0, 2.0]);
    string[] sta;
...
void printCommandLineHelp() {
    ...
    foreach (opt; g_options) {
        ...
        string optionString(string name) {
            if (name.length == 1)
                return "-" ~ name ~ (opt.hasValue ? " <value>" : "");
            else
                return "--" ~ name ~ (opt.hasValue ? "=<value>" : "");
        }
        ...
        string[] lopts;
        foreach (lo; longopts)
            lopts ~= optionString(lo);
        auto optstr = format(" %s %s", shortopt, lopts.join(", "));
        if (optstr.length < dcolumn)
            optstr ~= replicate(" ", dcolumn - optstr.length);

        auto indent = replicate(" ", dcolumn + 1);
        auto desc = wrap(opt.helpText, ncolumns - dcolumn - 2,
                optstr.length > dcolumn ? indent : "", indent).stripRight();

        if (optstr.length > dcolumn)
            logInfo("%s\n%s", optstr, desc);
        else
            logInfo("%s %s", optstr, desc);
    }
}
...
private void init() {
    ...
    foreach (spath; searchpaths) {
        ...
        if (cpath.exists) {
            scope (failure)
                logError("Failed to parse config file %s.", cpath);
...
private T fromValue(T)(JSONValue val) {
    ...
    static if (is(T == bool))
        ...
        return val.integer.to!T;
    else static if (is(T : double))
        return val.floating.to!T;
