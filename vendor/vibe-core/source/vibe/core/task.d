...
struct Task {
    ...
    private this(TaskFiber fiber, size_t task_counter) @safe nothrow {
        () @trusted{ m_fiber = cast(shared) fiber; }();
        m_taskCounter = task_counter;
    }

    this(in Task other) @safe nothrow {
	...
	*/
    static Task getThis() @safe nothrow {
        ...
        static if (__VERSION__ <= 2066)
            scope (failure)
                assert(0, "Internal error: function should be nothrow");
    ...
    nothrow {
        package @property inout(TaskFiber) taskFiber() inout @system {
            return cast(inout(TaskFiber)) m_fiber;
        }

        @property inout(Fiber) fiber() inout @system {
            return this.taskFiber;
        }

        @property size_t taskCounter() const @safe {
            return m_taskCounter;
        }

        @property inout(Thread) thread() inout @trusted {
            if (m_fiber)
                return this.taskFiber.thread;
            return null;
        }
		...
		*/
        ...
        const @trusted {
            ...
            try
                if (this.taskFiber.state == Fiber.State.TERM)
                    return false;
            catch (Throwable) {
            }
        ...
        package @property ref ThreadInfo tidInfo() @system {
            return m_fiber ? taskFiber.tidInfo : s_tidInfo;
        } // FIXME: this is not thread safe!
        package @property ref const(ThreadInfo) tidInfo() const @system {
            return m_fiber ? taskFiber.tidInfo : s_tidInfo;
        } // FIXME: this is not thread safe!
        ...
        /** Gets the `Tid` associated with this task for use with
			`std.concurrency`.
		*/
        @property Tid tid() @trusted {
            return tidInfo.ident;
        }
        /// ditto
        @property const(Tid) tid() const @trusted {
            return tidInfo.ident;
        }
    }

    T opCast(T)() const @safe nothrow if (is(T == bool)) {
        return m_fiber !is null;
    }
    ...
    void joinUninterruptible() @trusted nothrow {
        if (running)
            taskFiber.join!false(m_taskCounter);
    } // FIXME: this is NOT thread safe
    void interrupt() @trusted nothrow {
        if (running)
            taskFiber.interrupt(m_taskCounter);
    } // FIXME: this is NOT thread safe

    string toString() const @safe {
        import std.string;

        return format("%s:%s", () @trusted{ return cast(void*) m_fiber; }(), m_taskCounter);
    }
    ...
    void getDebugID(R)(ref R dst) {
        ...
        MD5 md;
        md.start();
        md.put(nativeToLittleEndian(() @trusted{
                return cast(size_t) cast(void*) m_fiber;
            }()));
...
struct TaskLocal(T) {
    ...
    @property ref T storage() @safe {
        ...
        auto data = () @trusted{
            return fiber.m_fls.ptr[m_offset .. m_offset + T.sizeof];
        }();
        if (!()@trusted{ return fiber.m_flsInit[m_id]; }()) {
            ...
            if (m_hasInitValue) {
                static if (__traits(compiles, ()@trusted{
                        emplace!T(data, m_initValue);
                    }()))
                    () @trusted{ emplace!T(data, m_initValue); }();
                else
                    assert(false, "Cannot emplace initialization value for type " ~ T.stringof);
            } else
...
final package class TaskFiber : Fiber {
    ...
    static TaskFiber getThis() @safe nothrow {
        auto f = () @trusted nothrow{ return Fiber.getThis(); }();
        if (auto tf = cast(TaskFiber) f)
            return tf;
        if (!ms_globalDummyFiber)
            ms_globalDummyFiber = new TaskFiber;
    ...
    static if (!__traits(compiles, ()@safe{ return Fiber.init.state; }()))
        @property State state() @trusted const nothrow {
            return super.state;
        }
    ...
    private void run() nothrow {
        ...
        import vibe.core.core : isEventLoopRunning, recycleFiber, taskScheduler,
            yield;
        ...
        try {
            while (true) {
                ...
                try {
                    ...
                    debug if (ms_taskEventCallback)
                        ms_taskEventCallback(TaskEvent.start, handle);
                    ...
                    task.call();
                    debug if (ms_taskEventCallback)
                        ms_taskEventCallback(TaskEvent.end, handle);
                }
                catch (Exception e) {
                    debug if (ms_taskEventCallback)
                        ms_taskEventCallback(TaskEvent.fail, handle);
                    import std.encoding;
                ...
                foreach (size_t i, ref bool b; m_flsInit) {
                    if (b) {
                        if (ms_flsInfo !is null && ms_flsInfo.length >= i
                                && ms_flsInfo[i] != FLSInfo.init)
	...
	*/
    @property inout(Thread) thread() inout @safe nothrow {
        return m_thread;
    }

    /** Returns the handle of the current Task running on this fiber.
	*/
    @property Task task() @safe nothrow {
        return Task(this, m_taskCounter);
    }

    @property ref inout(ThreadInfo) tidInfo() inout @safe nothrow {
        return m_tidInfo;
    }

    @property size_t taskCounter() const @safe nothrow {
        return m_taskCounter;
    }
	...
	*/
    void join(bool interruptiple)(size_t task_counter) @trusted {
	...
	*/
    void interrupt(size_t task_counter) @safe nothrow {
        ...
        if (caller != Task.init) {
            ...
            assert(caller.thread is this.thread,
                    "Interrupting tasks in different threads is not yet supported.");
        } else
            assert(() @trusted{ return Thread.getThis(); }() is this.thread,
                    "Interrupting tasks in different threads is not yet supported.");
        debug (VibeTaskLog)
    ...
    void bumpTaskCounter() @safe nothrow {
        import core.atomic : atomicOp;

        () @trusted{ atomicOp!"+="(this.m_taskCounter, 1); }();
    }

    package void handleInterrupt(scope void delegate() @safe nothrow on_interrupt) @safe nothrow {
    ...
    package void handleInterrupt() @safe {
        if (m_interrupt) {
            m_interrupt = false;
...
package struct TaskFuncInfo {
    ...
    void set(CALLABLE, ARGS...)(ref CALLABLE callable, ref ARGS args) {
        ...
        static assert(CALLABLE.sizeof <= TaskFuncInfo.callable.length,
                "Storage required for task callable is too large ("
                ~ CALLABLE.sizeof ~ " vs max " ~ callable.length ~ "): " ~ CALLABLE.stringof);
        ...
        () @trusted{
            static if (hasElaborateAssign!CALLABLE)
                initCallable!CALLABLE();
            static if (hasElaborateAssign!TARGS)
                initArgs!TARGS();
            typedCallable!CALLABLE = callable;
            foreach (i, A; ARGS) {
                static if (needsMove!A)
                    args[i].move(typedArgs!TARGS.expand[i]);
                else
                    typedArgs!TARGS.expand[i] = args[i];
            }
        }();
    }
    ...
    void initCallable(C)() nothrow {
        static const C cinit;
        this.callable[0 .. C.sizeof] = cast(void[])(&cinit)[0 .. 1];
    }

    void initArgs(A)() nothrow {
        static const A ainit;
        this.args[0 .. A.sizeof] = cast(void[])(&ainit)[0 .. 1];
    }
}

private ulong callPointer(C)(ref C callable) @trusted nothrow @nogc {
    alias IP = ulong;
    static if (is(C == function))
        return cast(IP) cast(void*) callable;
    else static if (is(C == delegate))
        return cast(IP) callable.funcptr;
    else static if (is(typeof(&callable.opCall) == function))
        return cast(IP) cast(void*)&callable.opCall;
    else static if (is(typeof(&callable.opCall) == delegate))
        return cast(IP)(&callable.opCall).funcptr;
    else
        return cast(IP)&callable;
}
...
@safe:
    ...
    @property size_t scheduledTaskCount() const nothrow {
	...
	*/
    void yield() {
        auto t = Task.getThis();
        if (t == Task.init)
            return; // not really a task -> no-op
        auto tf = () @trusted{ return t.taskFiber; }();
        debug (VibeTaskLog)
            logTrace("Yielding (interrupt=%s)", tf.m_interrupt);
        tf.handleInterrupt();
        if (tf.m_queue !is null)
            return; // already scheduled to be resumed
...
nothrow:
	...
	*/
    ExitReason process() {
        assert(TaskFiber.getThis().m_yieldLockCount == 0,
                "May not process events within an active yieldLock()!");
        ...
        while (true) {
            ...
            case ExitReason.timeout:
                if (!scheduledTaskCount)
                    return any_events
                        || any_tasks_processed ? ExitReason.idle : ExitReason.timeout;
                break;
	...
	*/
    ExitReason waitAndProcess() {
        ...
        final switch (er) {
        case ExitReason.exited, ExitReason.outOfWaiters:
            return er;
        case ExitReason.idle:
            return ExitReason.idle;
        case ExitReason.timeout:
	...
	*/
    void hibernate() {
        ...
        if (thist == Task.init) {
            assert(!isEventLoopRunning,
                    "Event processing outside of a fiber should only happen before the event loop is running!?");
	...
	*/
    void switchTo(Task t, Flag!"defer" defer = No.defer) {
        ...
        auto thisthr = thist ? thist.thread : () @trusted{
        ...
        if (tf.m_queue) {
            ...
            assert(tf.m_queue is &m_taskQueue,
                    "Task is already enqueued, but not in the main task queue.");
        ...
        if (thist == Task.init && defer == No.defer) {
            ...
            assert(!thistf || !thistf.m_queue,
                    "Calling task is running, but scheduled to be resumed!?");
	...
	*/
    ScheduleStatus schedule() nothrow {
        if (m_taskQueue.empty)
            return ScheduleStatus.idle;

        if (!m_markerTask)
            m_markerTask = new TaskFiber; // TODO: avoid allocating an actual task here!

        scope (exit)
            assert(!m_markerTask.m_queue, "Marker task still in queue!?");

        assert(Task.getThis() == Task.init,
                "TaskScheduler.schedule() may not be called from a task!");
        ...
        while (m_taskQueue.front !is m_markerTask) {
            ...
            if (m_taskQueue.empty)
                return ScheduleStatus.idle; // handle gracefully in release mode
        }
    ...
    private void resumeTask(Task t) nothrow {
        import std.encoding : sanitize;

        debug (VibeTaskLog)
            logTrace("task fiber resume");
        auto uncaught_exception = () @trusted nothrow{
            return t.fiber.call!(Fiber.Rethrow.no)();
        }();
        debug (VibeTaskLog)
        ...
        if (uncaught_exception) {
            ...
            logDebug("Full error: %s", () @trusted{
                return th.toString().sanitize;
            }());

            // always pass Errors on
            if (auto err = cast(Error) th)
                throw err;
        }
    }

    private void doYield(Task task) {
        assert(() @trusted{ return task.taskFiber; }().m_yieldLockCount == 0,
                "May not yield while in an active yieldLock()!");
        debug if (TaskFiber.ms_taskEventCallback)
            () @trusted{ TaskFiber.ms_taskEventCallback(TaskEvent.yield, task); }();
        () @trusted{ Fiber.yield(); }();
        debug if (TaskFiber.ms_taskEventCallback)
            () @trusted{ TaskFiber.ms_taskEventCallback(TaskEvent.resume, task); }();
