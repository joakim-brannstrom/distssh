...
Path relativeTo(Path)(Path path, Path base_path) @safe 
    ...
    if (is(Path == WindowsPath)) { // FIXME: this shouldn't be a special case here!
        bool samePrefix(size_t n) {
            return path.bySegment.map!(n => n.name).take(n)
                .equal(base_path.bySegment.map!(n => n.name).take(n));
        }
    ...
    if (path.endsWithSlash) {
        if (ret.empty)
            return Path("." ~ path.toString()[$ - 1]);
        else
            ret.endsWithSlash = true;
    }
    return ret;
}
...
unittest {
    ...
    assert(PosixPath("/some/path/")
            .relativeTo(PosixPath("/some/other/path/")) == PosixPath("../../path/"));
    assert(PosixPath("/some/path/")
            .relativeTo(PosixPath("/some/other/path")) == PosixPath("../../path/"));
    ...
    assert(WindowsPath("C:\\some\\path\\")
            .relativeTo(WindowsPath("C:\\some\\other\\path/")) == WindowsPath("..\\..\\path\\"));
    assert(WindowsPath("C:\\some\\path\\")
            .relativeTo(WindowsPath("C:\\some\\other\\path")) == WindowsPath("..\\..\\path\\"));

    assert(WindowsPath("\\\\server\\share\\some\\path")
            .relativeTo(WindowsPath("\\\\server\\share\\")) == WindowsPath("some\\path"));
    assert(WindowsPath("\\\\server\\share\\some\\path\\").relativeTo(WindowsPath(
            "\\\\server\\share\\some\\other\\path/")) == WindowsPath("..\\..\\path\\"));
    assert(WindowsPath("\\\\server\\share\\some\\path\\").relativeTo(WindowsPath(
            "\\\\server\\share\\some\\other\\path")) == WindowsPath("..\\..\\path\\"));
    ...
    assert(WindowsPath("\\\\server\\some\\path\\")
            .relativeTo(WindowsPath("C:\\some\\other\\path")) == WindowsPath(
                "\\\\server\\some\\path\\"));
    assert(WindowsPath("\\\\server\\some\\path\\")
            .relativeTo(WindowsPath("\\\\otherserver\\path")) == WindowsPath(
                "\\\\server\\some\\path\\"));
    assert(WindowsPath("\\some\\path\\")
            .relativeTo(WindowsPath("\\other\\path")) == WindowsPath("..\\..\\some\\path\\"));

    assert(WindowsPath("\\\\server\\share\\path1")
            .relativeTo(WindowsPath("\\\\server\\share\\path2")) == WindowsPath("..\\path1"));
    assert(WindowsPath("\\\\server\\share\\path1").relativeTo(WindowsPath(
            "\\\\server\\share2\\path2")) == WindowsPath("\\\\server\\share\\path1"));
    assert(WindowsPath("\\\\server\\share\\path1").relativeTo(WindowsPath(
            "\\\\server2\\share2\\path2")) == WindowsPath("\\\\server\\share\\path1"));
}
...
unittest {
    ...
    { // relative paths across Windows devices are not allowed
        auto p1 = WindowsPath("\\\\server\\share");
        assert(p1.absolute);
        auto p2 = WindowsPath("\\\\server\\othershare");
        assert(p2.absolute);
        auto p3 = WindowsPath("\\\\otherserver\\share");
        assert(p3.absolute);
        auto p4 = WindowsPath("C:\\somepath");
        assert(p4.absolute);
        auto p5 = WindowsPath("C:\\someotherpath");
        assert(p5.absolute);
        auto p6 = WindowsPath("D:\\somepath");
        assert(p6.absolute);
        auto p7 = WindowsPath("\\\\server\\share\\path");
        assert(p7.absolute);
        auto p8 = WindowsPath("\\\\server\\share\\otherpath");
        assert(p8.absolute);
...
Path relativeToWeb(Path)(Path path, Path base_path) @safe 
    ...
    if (!base_path.endsWithSlash) {
        ...
        if (base_path.hasParentPath)
            base_path = base_path.parentPath;
        else
            base_path = Path("/");
...
deprecated("Use NativePath or one the specific path types instead.") alias Path = NativePath;
deprecated("Use NativePath.Segment or one the specific path types instead.") alias PathEntry = Path
    .Segment;
...
@safe:
    ...
    @safe:
		...
		*/
        this(string name, char separator = '\0') {
            ...
            enforce!PathValidationException(separator == '\0'
                    || Format.isSeparator(separator), "Invalid path separator.");
        ...
        /** Constructs a path segment without performing validation.
			...
			Params:
				...
				separator = Optional trailing path separator (e.g. `'/'`)
		*/
        static Segment fromTrustedString(string name, char separator = '\0') nothrow @nogc pure {
            import std.algorithm.searching : any;
        ...
        deprecated("Use the constructor instead.") static Segment validateFilename(string name) {
            return Segment(name);
        }
        ...
        @property char separator() const nothrow @nogc {
            return m_separator;
        }
        /// ditto
        ...
        deprecated("Use .name instead.") string toString() const nothrow @nogc {
            return m_name;
        }
		...
		*/
        GenericPath!F.Segment opCast(T : GenericPath!F.Segment, F)() {
            char dsep = '\0';
            if (m_separator) {
                if (F.isSeparator(m_separator))
                    dsep = m_separator;
                else
                    dsep = F.defaultSeparator;
            }
            return GenericPath!F.Segment(m_name, dsep);
        }

        /// Compares two path segment names
        bool opEquals(Segment other) const nothrow @nogc {
            return this.name == other.name && this.hasSeparator == other.hasSeparator;
        }
        /// ditto
        bool opEquals(string name) const nothrow @nogc {
	...
	*/
    static struct PathRange {
        ...
        @property bool empty() const nothrow @nogc {
            return m_path.length == 0 && m_fronts.empty;
        }
        ...
        @property Segment front() {
            return m_fronts.front;
        }
	...
	*/
    ...
    /** Constructs a path from an input range of `Segment`s.
		...
		Throws:
			...
			throw an exception.
	*/
    this(R)(R segments) if (isInputRange!R && is(ElementType!R : Segment)) {
	...
	*/
    static GenericPath fromTrustedString(string p) nothrow @nogc {
    ...
    static bool isSeparator(dchar ch) {
        return ch < 0x80 && Format.isSeparator(cast(char) ch);
    }
    ...
    @property bool absolute() const nothrow @nogc {
        return Format.getAbsolutePrefix(m_path).length > 0;
    }

    /// Determines whether the path ends with a path separator (i.e. represents a folder specifically).
    @property bool endsWithSlash() const nothrow @nogc {
        return m_path.length > 0 && Format.isSeparator(m_path[$ - 1]);
    }
    /// ditto
    @property void endsWithSlash(bool v) nothrow {
        bool ews = this.endsWithSlash;
        if (!ews && v)
            m_path ~= Format.defaultSeparator;
        else if (ews && !v)
            m_path = m_path[0 .. $ - 1]; // FIXME?: "/test//" -> "/test/"
    }

    /// Iterates over the path by `Segment`.
    @property PathRange bySegment() const {
        return PathRange(m_path);
    }

    /// Returns the trailing segment of the path.
    @property Segment head() const {
	...
	*/
    void normalize() {
        ...
        foreach (n; this.bySegment) {
            if (n.hasSeparator)
                n.separator = Format.defaultSeparator;
            if (!got_non_sep) {
                if (n.name == "")
                    newnodes ~= n;
                else
            ...
            case "..":
                ...
                if (newnodes.length > 0 && newnodes[$ - 1].name != "..")
    ...
    hash_t toHash() const nothrow @trusted {
        try
            return typeid(string).getHash(&m_path);
        catch (Exception e)
            assert(false, "getHash for string throws!?");
    }
    ...
    /** Compares two path objects.
		...
		normalized first.
	*/
    bool opEquals(GenericPath other) const @nogc {
	...
	*/
    P opCast(P)() const if (isInstanceOf!(.GenericPath, P)) {
        static if (is(P == GenericPath))
            return this;
        else
            return P(this.bySegment.map!(n => cast(P.Segment) n));
    }
	...
	*/
    GenericPath opBinary(string op : "~")(string subpath) const {
        return this ~ GenericPath(subpath);
    }
    /// ditto
    GenericPath opBinary(string op : "~")(Segment subpath) const {
        return this ~ GenericPath(subpath);
    }
    /// ditto
    GenericPath opBinary(string op : "~", F)(GenericPath!F.Segment subpath) const {
        return this ~ cast(Segment)(subpath);
    }
    /// ditto
    GenericPath opBinary(string op : "~")(GenericPath subpath) const nothrow {
        ...
        if (endsWithSlash || empty)
            return GenericPath.fromTrustedString(m_path ~ subpath.m_path);
        else
            return GenericPath.fromTrustedString(m_path ~ Format.defaultSeparator ~ subpath.m_path);
    }
    /// ditto
    GenericPath opBinary(string op : "~", F)(GenericPath!F subpath) const 
            if (!is(F == Format)) {
        return this ~ cast(GenericPath) subpath;
    }
    /// ditto
    ...
    void opOpAssign(string op : "~", T)(T op) {
        this = this ~ op;
    }
	...
	*/
    bool startsWith(GenericPath prefix) const nothrow {
...
unittest {
    ...
    assert(PosixPath("hello/world").bySegment.equal([PosixPath.Segment("hello",
            '/'), PosixPath.Segment("world")]));
    assert(PosixPath("/hello/world/").bySegment.equal([PosixPath.Segment("",
            '/'), PosixPath.Segment("hello", '/'), PosixPath.Segment("world", '/')]));
    assert(PosixPath("hello\\world").bySegment.equal([PosixPath.Segment("hello\\world")]));
    assert(WindowsPath("hello/world").bySegment.equal([WindowsPath.Segment("hello",
            '/'), WindowsPath.Segment("world")]));
    assert(WindowsPath("/hello/world/").bySegment.equal([WindowsPath.Segment("",
            '/'), WindowsPath.Segment("hello", '/'), WindowsPath.Segment("world", '/')]));
    assert(WindowsPath("hello\\w/orld").bySegment.equal([WindowsPath.Segment("hello",
            '\\'), WindowsPath.Segment("w", '/'), WindowsPath.Segment("orld")]));
    assert(WindowsPath("hello/w\\orld").bySegment.equal([WindowsPath.Segment("hello",
            '/'), WindowsPath.Segment("w", '\\'), WindowsPath.Segment("orld")]));
}
...
unittest {
...

    {
        ...
        uncp.normalize();
        version (Windows)
            assert(uncp.toNativeString() == unc);
        assert(uncp.absolute);
...

    {
        ...
        version (Windows) {
            ...
            assert(abspathp.toNativeString() == abspath);
...

    {
        ...
        version (Windows)
            assert(relpathp.toNativeString() == "test/path/");
        else
            assert(relpathp.toNativeString() == relpath);
...

    {
        ...
        assert((cast(PosixPath) winpathp).toString() == "/C:/windows/test",
                (cast(PosixPath) winpathp).toString());
        version (Windows)
    ...
    assert((WindowsPath("C:\\Windows") ~ InetPath("test/this"))
            .toString() == "C:\\Windows\\test/this");
    assert(InetPath("/C:/Windows").absolute);
    assert((cast(WindowsPath) InetPath("/C:/Windows")).toString() == "C:/Windows");
    assert((InetPath("/C:/Windows") ~ WindowsPath("test\\this"))
            .toString() == "/C:/Windows/test/this");
...
@safe unittest {
    ...
    void test1(PosixPath p) {
        app.put(p);
    }

    void test2(PosixPath[] ps) {
        app.put(ps);
    }
...
unittest {
    assert(WindowsPath([WindowsPath.Segment("foo"),
            WindowsPath.Segment("bar")]).toString() == "foo\\bar");
}

/// Thrown when an invalid string representation of a path is detected.
class PathValidationException : Exception {
    this(string text, string file = __FILE__, size_t line = cast(size_t) __LINE__,
...
struct WindowsPathFormat {
    static void toString(I, O)(I segments, O dst)
        ...
        char sep(char s) {
            return isSeparator(s) ? s : defaultSeparator;
        }
    ...
    unittest {
        ...
        struct Segment {
            string name;
            char separator = 0;
            static Segment fromTrustedString(string str, char sep = 0) pure nothrow @nogc {
                return Segment(str, sep);
            }
        }

        string str(Segment[] segs...) {
...
@safe nothrow pure:
    ...
    static bool isSeparator(dchar ch) @nogc {
        import std.algorithm.comparison : among;

        return ch.among!('\\', '/') != 0;
    }

    static string getAbsolutePrefix(string path) @nogc {
        if (!path.length)
            return null;
    ...
    static string getFrontNode(string path) @nogc {
    ...
    static string getBackNode(string path) @nogc {
        if (!path.length)
            return path;
    ...
    static auto decodeSegment(S)(string segment) {
        ...
        char sep = '\0';
        if (!segment.length)
            return R(S.fromTrustedString(null));
    ...
    static string validatePath(string path) @nogc {
        import std.algorithm.comparison : among;
    ...
    static string validateDecodedSegment(string segment) @nogc {
...
@safe nothrow pure:
    ...
    static auto decodeSegment(S)(string segment) {
        ...
        if (!segment.length)
            return only(S.fromTrustedString(null, '/'));
        if (segment[$ - 1] == '/')
...
@safe pure nothrow:
        ...
        for (size_t i = 0; i < path.length; i++) {
            ...
            case '%': // pct encoding
                ...
                foreach (j; 0 .. 2) {
                    switch (path[++i]) {
                    default:
                        return "Invalid percent encoding sequence in internet path.";
                    case '0': .. case '9':
                    case 'a': .. case 'f':
                    case 'A': .. case 'F':
                        break;
    ...
    static string validateDecodedSegment(string seg) @nogc {
        return null;
    }
