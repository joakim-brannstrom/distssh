...
final class ConnectionPool(Connection) {
    ...
    deprecated("Use an @safe callback instead") this(
            Connection delegate() connection_factory, uint max_concurrent = uint.max) @system {
	...
	*/
    LockedConnection!Connection lockConnection() @safe {
        debug assert(m_thread is() @trusted{ return Thread.getThis(); }(),
                "ConnectionPool was called from a foreign thread!");
        ...
        Connection conn;
        if (cidx != size_t.max) {
            logTrace("returning %s connection %d of %d", Connection.stringof,
                    cidx, m_connections.length);
            conn = m_connections[cidx];
        } else {
            logDebug("creating new %s connection, all %d are in use",
                    Connection.stringof, m_connections.length);
...
@safe:
    ...
    this(this) {
        ...
        if (!!m_conn) {
            ...
            static if (is(typeof(cast(void*) conn)))
                logTrace("conn %s copy %d", () @trusted{
                    return cast(void*) m_conn;
                }(), m_pool.m_lockCount[m_conn]);
        }
    }
    ...
    @property int __refCount() const {
        return m_pool.m_lockCount.get(m_conn, 0);
    }

    @property inout(Connection) __conn() inout {
        return m_conn;
    }
