...
FileStream createTempFile(string suffix = null) {
    version (Windows) {
        ...
        auto fd = () @trusted{
            return mkstemps(templ.ptr, cast(int) suffix.length);
        }();
...
bool existsFile(string path) nothrow {
    ...
    static if (__VERSION__ < 2067)
        scope (failure)
            assert(0, "Error: existsFile should never throw");
...
*/
FileInfo getFileInfo(NativePath path) @trusted {
...
*/
void listDirectory(NativePath path, scope bool delegate(FileInfo info) del) @trusted {
...
int delegate(scope int delegate(ref FileInfo)) iterateDirectory(NativePath path) {
    int iterator(scope int delegate(ref FileInfo) del) {
        int ret = 0;
        listDirectory(path, (fi) { ret = del(fi); return ret == 0; });
        return ret;
    }
...
@safe:
    ...
    @property int fd() {
        return cast(int) m_fd;
    }

    /// The path of the file.
    @property NativePath path() const {
        return ctx.path;
    }

    /// Determines if the file stream is still open
    @property bool isOpen() const {
        return m_fd != FileFD.invalid;
    }
    ...
    @property bool readable() const nothrow {
        return ctx.mode != FileMode.append;
    }

    @property bool writable() const nothrow {
        return ctx.mode != FileMode.read;
    }
    ...
    @property bool empty() const {
        ...
        return ctx.ptr >= ctx.size;
    }

    @property ulong leastSize() const {
        assert(this.readable);
        return ctx.size - ctx.ptr;
    }
    ...
    size_t read(ubyte[] dst, IOMode mode) {
        auto res = asyncAwait!(FileIOCallback, cb => eventDriver.files.read(m_fd,
                ctx.ptr, dst, mode, cb), cb => eventDriver.files.cancelRead(m_fd));
        ctx.ptr += res[2];
    ...
    size_t write(in ubyte[] bytes, IOMode mode) {
        auto res = asyncAwait!(FileIOCallback, cb => eventDriver.files.write(m_fd,
                ctx.ptr, bytes, mode, cb), cb => eventDriver.files.cancelWrite(m_fd));
        ctx.ptr += res[2];
        if (ctx.ptr > ctx.size)
            ctx.size = ctx.ptr;
        enforce(res[1] == IOStatus.ok, "Failed to read data from disk.");
    ...
    private inout(CTX)* ctx() inout nothrow {
        return m_ctx;
    }
}
...
private void writeDefault(OutputStream, InputStream)(ref OutputStream dst,
        InputStream stream, ulong nbytes = ulong.max)
    ...
    static struct Buffer {
        ubyte[64 * 1024] bytes = void;
    }

    auto bufferobj = () @trusted{ return theAllocator.make!Buffer(); }();
    scope (exit)
        () @trusted{ theAllocator.dispose(bufferobj); }();
...
@safe:
    ...
    private static struct Context {
        ...
        void onChange(WatcherID, in ref FileChange change) nothrow {
            DirectoryChangeType ct;
            final switch (change.kind) {
            case FileChangeKind.added:
                ct = DirectoryChangeType.added;
                break;
            case FileChangeKind.removed:
                ct = DirectoryChangeType.removed;
                break;
            case FileChangeKind.modified:
                ct = DirectoryChangeType.modified;
                break;
            }

            static if (is(typeof(change.baseDirectory))) {
                // eventcore 0.8.23 and up
                this.changes ~= DirectoryChange(ct, NativePath.fromTrustedString(
                        change.baseDirectory) ~ NativePath.fromTrustedString(
                        change.directory) ~ NativePath.fromTrustedString(change.name.idup));
            } else {
                this.changes ~= DirectoryChange(ct, NativePath.fromTrustedString(
                        change.directory) ~ NativePath.fromTrustedString(change.name.idup));
            }
    ...
    private this(NativePath path, bool recursive) {
        ...
        m_watcher = eventDriver.watchers.watchDirectory(path.toNativeString,
                recursive, &m_context.onChange);
        m_context.path = path;
    ...
    this(this) nothrow {
        if (m_watcher != WatcherID.invalid)
            eventDriver.watchers.addRef(m_watcher);
    }
    ...
    @property NativePath path() const nothrow {
        return m_context.path;
    }

    /// Indicates if the directory is watched recursively
    @property bool recursive() const nothrow {
        return m_context.recursive;
    }
	...
	*/
    bool readChanges(ref DirectoryChange[] dst, Duration timeout = Duration.max) {
        if (timeout == Duration.max) {
            ...
            if (!m_context.changes.data.length)
                return false;
        }
...
private FileInfo makeFileInfo(DirEntry ent) @trusted {
    FileInfo ret;
    auto fullname = ent.name.endsWith('/') || ent.name.endsWith('\\') ? ent.name[0 .. $ - 1]
        : ent.name;
    ...
    version (Windows)
        ...
        ret.timeCreated = ent.timeLastModified;
    ...
    version (Windows) {
        ...
        ret.hidden = ret.name.startsWith('.');
    return ret;
}
