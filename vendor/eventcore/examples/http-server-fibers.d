...
size_t storeSize = 0;
Fiber getFiber() nothrow {

    if (storeSize > 0)
        return store[--storeSize];
    return new Fiber({  });
}

void done(Fiber f) nothrow {
...
@safe:
    ...
    void finish(const(Exception) e = null) nothrow {
        ...
        () @trusted{
            scope (failure)
                assert(false);
            if (owner.state == Fiber.State.HOLD)
                owner.call();
        }();
    }
}
...
@safe: /*@nogc:*/
    ...
    this(StreamSocketFD sock, ubyte[] buffer) nothrow {
        m_socket = sock;
        m_readBuffer = buffer;
    }
    ...
    ubyte[] readLine() {
        reader.start();
        if (m_readBufferFill >= 2)
            onReadLineData(m_socket, IOStatus.ok, 0);
        else
            eventDriver.sockets.read(m_socket,
                    m_readBuffer[m_readBufferFill .. $], IOMode.once, &onReadLineData);
        reader.wait();
        auto ln = m_line;
        m_line = null;
        return ln;
    ...
    private void onWrite(StreamSocketFD fd, IOStatus status, size_t len) @safe nothrow {
        static const ex = new Exception("Failed to write data!");
        writer.finish(status == IOStatus.ok ? null : ex);
    }

    private void onData(StreamSocketFD, IOStatus status, size_t bytes_read) @safe nothrow {
        if (status != IOStatus.ok)
            m_empty = true;
        reader.finish();
    }

    private void onReadLineData(StreamSocketFD, IOStatus status, size_t bytes_read) @safe nothrow {
        ...
        if (idx >= 0) {
            assert(m_readBufferFill + idx <= m_readBuffer.length,
                    "Not enough space to buffer the incoming line.");
