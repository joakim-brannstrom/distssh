...
@safe nothrow:
	...
	*/
    void reserve(size_t count) @safe {
    ...
    /** Consumes all elements of the queue and returns a range containing the
		...
		returned range.
	*/
    ConsumedRange consume() @safe {
        if (!m_pendingCount)
            return ConsumedRange(null, 0, 0);
    ...
    T consumeOne() {
        ...
        if (m_consumedCount)
            ...
            m_first = (m_first + 1) & m_capacityMask;
        m_pendingCount--;
        return ret;
    }
    ...
    nothrow:
        ...
        @property ref inout(T) front() inout {
            return m_queue.m_storage[m_first & m_queue.m_capacityMask].value;
        }
        ...
        ref inout(T) opIndex(size_t idx) inout {
            return m_queue.m_storage[(m_first + idx) & m_queue.m_capacityMask].value;
        }
...
void filterPending(alias pred, T)(ConsumableQueue!T q) {
    ...
    while (ir < q.m_pendingCount) {
        if (!pred(q.getPendingAt(ir))) {
            ...
            if (ir != iw)
                q.getPendingAt(iw) = q.getPendingAt(ir);
            iw++;
        }
        ir++;
    }
