...
@safe: /*@nogc:*/ nothrow:
    ...
    private {
        ...
        version (linux)
            alias SignalsDriver = SignalFDEventDriverSignals!Loop;
        else
            alias SignalsDriver = DummyEventDriverSignals!Loop;
        alias TimerDriver = LoopTimeoutTimerDriver;
        alias SocketsDriver = PosixEventDriverSockets!Loop;
        version (Windows)
            alias DNSDriver = EventDriverDNS_GHBN!(EventsDriver, SignalsDriver);
        ...
        version (linux)
            alias WatcherDriver = InotifyEventDriverWatchers!EventsDriver;
        //else version (OSX) alias WatcherDriver = FSEventsEventDriverWatchers!EventsDriver;
        else
            alias WatcherDriver = PollEventDriverWatchers!EventsDriver;
    ...
    final override @property inout(CoreDriver) core() inout {
        return m_core;
    }

    final override @property shared(inout(CoreDriver)) core() shared inout {
        return m_core;
    }

    final override @property inout(EventsDriver) events() inout {
        return m_events;
    }

    final override @property shared(inout(EventsDriver)) events() shared inout {
        return m_events;
    }

    final override @property inout(SignalsDriver) signals() inout {
        return m_signals;
    }

    final override @property inout(TimerDriver) timers() inout {
        return m_timers;
    }

    final override @property inout(SocketsDriver) sockets() inout {
        return m_sockets;
    }

    final override @property inout(DNSDriver) dns() inout {
        return m_dns;
    }

    final override @property inout(FileDriver) files() inout {
        return m_files;
    }

    final override @property inout(WatcherDriver) watchers() inout {
        return m_watchers;
    }
...
final class PosixEventDriverCore(Loop : PosixEventLoop, Timers
        : EventDriverTimers, Events : EventDriverEvents) : EventDriverCore {
@safe nothrow:
    ...
    @property size_t waiterCount() const {
        return m_loop.m_waiterCount + m_timers.pendingCount;
    }
    ...
    final override void runInOwnerThread(ThreadCallback del, intptr_t param) shared {
        ...
        try {
            synchronized (m)
                () @trusted{ return (cast() this).m_threadCallbacks; }().put(tuple(del, param));
        }
        catch (Exception e)
            assert(false, e.msg);
    ...
    protected final void* rawUserDataImpl(FD descriptor, size_t size,
...
nothrow:
    ...
    package {
        AlgebraicChoppedVector!(FDSlot, StreamSocketSlot, StreamListenSocketSlot,
                DgramSocketSlot, DNSSlot, WatcherSlot, EventSlot, SignalSlot) m_fds;
        size_t m_waiterCount = 0;
    }

    protected @property int maxFD() const {
        return cast(int) m_fds.length;
    }
    ...
    protected abstract void updateFD(FD fd, EventMask old_mask,
            EventMask new_mask, bool edge_triggered = true);
    ...
    package void setNotifyCallback(EventType evt)(FD fd, FDSlotCallback callback) {
        ...
        with (m_fds[fd.value]) {
            if (callback !is null) {
                if (!(common.flags & FDFlags.internal))
                    m_waiterCount++;
                common.refCount++;
            } else {
                common.refCount--;
    ...
    package void initFD(T)(FD fd, FDFlags flags, auto ref T slot_init) {
        with (m_fds[fd.value]) {
            ...
            assert(specific.kind == typeof(specific).Kind.none,
                    "Initializing slot that has not been cleared.");
            common.refCount = 1;
            common.flags = flags;
            specific = slot_init;
        }
    ...
    package final void* rawUserDataImpl(size_t descriptor, size_t size,
            DataInitializer initialize, DataInitializer destroy) @system {
        ...
        if (size > FDSlot.userData.length)
            assert(false);
...
private struct FDSlot {
    ...
    @property EventMask eventMask() const nothrow {
        ...
        if (callback[EventType.read]!is null)
            ret |= EventMask.read;
        if (callback[EventType.write]!is null)
            ret |= EventMask.write;
        if (callback[EventType.status]!is null)
            ret |= EventMask.status;
        return ret;
    }
}
...
void log(ARGS...)(string fmt, ARGS args) @trusted {
