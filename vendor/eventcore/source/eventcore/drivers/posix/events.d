...
final class PosixEventDriverEvents(Loop : PosixEventLoop, Sockets : EventDriverSockets)
    : EventDriverEvents {
    @safe: /*@nogc:*/ nothrow:
        private {
            Loop m_loop;
        ...
        this(Loop loop, Sockets sockets) {
            ...
            version (linux) {
                ...
                m_eventsMutex = new Mutex;
        }

        package @property Loop loop() {
            return m_loop;
        }
        ...
        package(eventcore) EventID createInternal(bool is_internal = true) {
            version (linux) {
                ...
                version (Posix) {
                    ...
                    import core.sys.posix.sys.socket : SOCK_DGRAM, AF_UNIX,
                        socketpair;
        ...
        final override void trigger(EventID event, bool notify_all) shared @trusted {
            import core.atomic : atomicStore;

            auto thisus = cast(PosixEventDriverEvents) this;
            assert(event < thisus.m_loop.m_fds.length,
                    "Invalid event ID passed to shared triggerEvent.");
            long one = 1;
            //log("emitting for all threads");
            if (notify_all)
                atomicStore(thisus.getSlot(event).triggerAll, true);
            version (Posix)
                .write(cast(int) event, &one, one.sizeof);
            else
                assert(send(cast(int) event, cast(const(ubyte*))&one, one.sizeof, 0) == one.sizeof);
        }
        ...
        private void onEvent(FD fd) @trusted {
        ...
        final override bool releaseRef(EventID descriptor) {
            ...
            if (--getRC(descriptor) == 0) {
                ...
                version (Posix)
                    ...
                    () @trusted{ closesocket(cast(SOCKET) descriptor); }();
                return false;
            }
            return true;
        }

        final protected override void* rawUserData(EventID descriptor,
