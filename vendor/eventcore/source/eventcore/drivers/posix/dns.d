...
version (Posix) final class EventDriverDNS_GAI(Events : EventDriverEvents,
    Signals : EventDriverSignals) : EventDriverDNS {
    ...
    override DNSLookupID lookupHost(string name, DNSLookupCallback on_lookup_finished) {
        ...
        try
            t.executeInNewThread(); //taskPool.put(t);
        catch (Exception e)
            return DNSLookupID.invalid;
        debug (EventCoreLogDNS)
            print("lookup handle: %s", handle);
    ...
    static void taskFun(Lookup* lookup, int af, shared(Events) events, EventID event) {
        debug (EventCoreLogDNS)
            print("lookup %s start", lookup.name);
        addrinfo hints;
        ...
        hints.ai_family = af;
        () @trusted{
            lookup.retcode = getaddrinfo(lookup.name.toStringz(), null,
                    af == AddressFamily.UNSPEC ? null : &hints, &lookup.result);
        }();
        events.trigger(event, true);
        debug (EventCoreLogDNS)
            print("lookup %s finished", lookup.name);
    }
    ...
    private void onDNSSignal(EventID event) @trusted nothrow {
        debug (EventCoreLogDNS)
            print("DNS event triggered");
        ...
        foreach (i, ref l; m_lookups) {
            ...
            if (l.callback) {
                if (l.result || l.retcode) {
                    debug (EventCoreLogDNS)
                        print("found finished lookup %s for %s", i, l.name);
                    auto cb = l.callback;
                    auto ai = l.result;
                    DNSStatus status;
                    switch (l.retcode) {
        ...
        debug (EventCoreLogDNS)
            print("Max active DNS handle: %s", m_maxHandle);
    }

    private DNSLookupID getFreeHandle() @safe nothrow {
...
final class EventDriverDNS_GAIA(Events : EventDriverEvents, Signals : EventDriverSignals)
    : EventDriverDNS {
    ...
    @safe nothrow:
        ...
        override DNSLookupID lookupHost(string name, DNSLookupCallback on_lookup_finished) {
            ...
            auto ret = () @trusted{
                return getaddrinfo_a(GAI_NOWAIT, &res, 1, &evt);
            }();
        ...
        private void onDNSSignal(SignalListenID, SignalStatus status, int signal) @safe nothrow {
...
final class EventDriverDNS_GHBN(Events : EventDriverEvents, Signals : EventDriverSignals)
    : EventDriverDNS {
        import std.parallelism : task, taskPool;
        import std.string : toStringz;
        ...
        override DNSLookupID lookupHost(string name, DNSLookupCallback on_lookup_finished) {
            ...
            default:
                assert(false, "Invalid address family returned from host lookup.");
            case AF_INET: {
                    sockaddr_in sa;
                    sa.sin_family = AF_INET;
                    sa.sin_addr = () @trusted{
                        return *cast(in_addr*) he.h_addr_list[0];
                    }();
                    scope addr = new RefAddress(() @trusted{
                        return cast(sockaddr*)&sa;
                    }(), sa.sizeof);
                    RefAddress[1] aa;
                    aa[0] = addr;
            ...
            case AF_INET6: {
                    ...
                    sa.sin6_addr = () @trusted{
                        return *cast(in6_addr*) he.h_addr_list[0];
                    }();
                    scope addr = new RefAddress(() @trusted{
                        return cast(sockaddr*)&sa;
                    }(), sa.sizeof);
...
private void passToDNSCallback()(DNSLookupID id, scope DNSLookupCallback cb,
        DNSStatus status, addrinfo* ai_orig) @trusted nothrow {
    import std.typecons : scoped;

    try {
        typeof(scoped!RefAddress())[16] addrs_prealloc = [
            scoped!RefAddress(), scoped!RefAddress(), scoped!RefAddress(),
            scoped!RefAddress(), scoped!RefAddress(), scoped!RefAddress(),
            scoped!RefAddress(), scoped!RefAddress(), scoped!RefAddress(),
            scoped!RefAddress(), scoped!RefAddress(), scoped!RefAddress(),
