...
version (FreeBSD)
    enum have_kqueue = true;
else version (DragonFlyBSD)
    enum have_kqueue = true;
else version (OSX)
    enum have_kqueue = true;
else
    enum have_kqueue = false;
...
version (OSX)
    ...
    static assert(false, "Kqueue not supported on this OS.");
...
final class KqueueEventLoop : PosixEventLoop {
    ...
    override bool doProcessEvents(Duration timeout) @trusted {
        ...
        auto ret = kevent(m_queue, m_changes.ptr, cast(int) m_changes.length,
                m_events.ptr, cast(int) m_events.length, timeout == Duration.max ? null : &ts);
        m_changes.length = 0;
