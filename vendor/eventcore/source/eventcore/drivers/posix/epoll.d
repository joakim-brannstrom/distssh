...
version (linux)  : public import eventcore.drivers.posix.driver;
...
nothrow:
    ...
    override bool doProcessEvents(Duration timeout) @trusted {
        import std.algorithm : min, max;

        //assert(Fiber.getThis() is null, "processEvents may not be called from within a fiber!");

        debug (EventCoreEpollDebug)
            print("Epoll wait %s, %s", m_events.length, timeout);
        long tomsec;
        if (timeout == Duration.max)
            tomsec = long.max;
        else
            tomsec = max((timeout.total!"hnsecs" + 9999) / 10_000, 0);
        auto ret = epoll_wait(m_epoll, m_events.ptr, cast(int) m_events.length,
                tomsec > int.max ? -1 : cast(int) tomsec);
        debug (EventCoreEpollDebug)
            print("Epoll wait done: %s", ret);

        if (ret > 0) {
            foreach (ref evt; m_events[0 .. ret]) {
                debug (EventCoreEpollDebug)
                    print("Epoll event on %s: %s", evt.data.fd, evt.events);
    ...
    override void registerFD(FD fd, EventMask mask, bool edge_triggered = true) {
        debug (EventCoreEpollDebug)
            print("Epoll register FD %s: %s", fd, mask);
        epoll_event ev;
        if (edge_triggered)
            ev.events |= EPOLLET;
    ...
    override void unregisterFD(FD fd, EventMask mask) {
        debug (EventCoreEpollDebug)
            print("Epoll unregister FD %s", fd);
    ...
    override void updateFD(FD fd, EventMask old_mask, EventMask mask, bool edge_triggered = true) {
        debug (EventCoreEpollDebug)
            print("Epoll update FD %s: %s", fd, mask);
        epoll_event ev;
        if (edge_triggered)
            ev.events |= EPOLLET;
