...
version (Posix) {
    ...
    import core.sys.posix.netdb : AI_ADDRCONFIG, AI_V4MAPPED, addrinfo,
...
version (linux) {
    ...
    static if (__VERSION__ < 2077) {
        ...
        import core.sys.linux.netinet.in_ : IP_ADD_MEMBERSHIP,
            IP_MULTICAST_LOOP;
}
version (OSX) {
    static if (__VERSION__ < 2077) {
        ...
        import core.sys.darwin.netinet.in_ : IP_ADD_MEMBERSHIP,
            IP_MULTICAST_LOOP;
}
version (FreeBSD) {
    static if (__VERSION__ < 2077) {
        ...
        import core.sys.freebsd.netinet.in_ : IP_ADD_MEMBERSHIP,
            IP_MULTICAST_LOOP;
}
version (DragonFlyBSD) {
    import core.sys.dragonflybsd.netinet.in_ : IP_ADD_MEMBERSHIP,
        IP_MULTICAST_LOOP;
}
version (Solaris) {
...
@safe: /*@nogc:*/ nothrow:
    ...
    final override StreamSocketFD connectStream(scope Address address,
            scope Address bind_address, ConnectCallback on_connect) {
        ...
        auto ret = () @trusted{
            return connect(cast(sock_t) sock, address.name, address.nameLen);
        }();
        if (ret == 0) {
    ...
    private void onConnect(FD fd) {
        ...
        with (m_loop.m_fds[sock].streamSocket) {
            ...
            connectCallback = null;
            if (cb)
                cb(sock, ConnectStatus.connected);
        }
    }
    ...
    final override StreamListenSocketFD listenStream(scope Address address,
        ...
        () @trusted{
            ...
            version (Windows) {
                ...
                if ((options & StreamListenOptions.reusePort) && setsockopt(sockfd,
                        SOL_SOCKET, SO_REUSEPORT, &tmp_reuse, tmp_reuse.sizeof) != 0) {
                    invalidateSocket();
                    return;
                }
            }
    ...
    private void onAccept(FD listenfd) {
        ...
        version (linux) {
            () @trusted{
                sockfd = accept4(cast(sock_t) listenfd, () @trusted{
            ...
            () @trusted{
                sockfd = accept(cast(sock_t) listenfd, () @trusted{
                    return cast(sockaddr*)&addr;
                }(), &addr_len);
            }();
            if (sockfd == -1)
        ...
        m_loop.m_fds[listenfd].streamListen.acceptCallback(
                cast(StreamListenSocketFD) listenfd, fd, addrc);
    }
    ...
    final override bool getLocalAddress(SocketFD sock, scope RefAddress dst) {
        ...
        if (()@trusted{
                return getsockname(cast(sock_t) sock, dst.name, &addr_len);
            }() != 0)
            return false;
        dst.cap(addr_len);
        return true;
    ...
    final override bool getRemoteAddress(SocketFD sock, scope RefAddress dst) {
        socklen_t addr_len = dst.nameLen;
        if (()@trusted{
                return getpeername(cast(sock_t) sock, dst.name, &addr_len);
            }() != 0)
            return false;
        dst.cap(addr_len);
        return true;
    ...
    final override void setTCPNoDelay(StreamSocketFD socket, bool enable) {
        int opt = enable;
        () @trusted{
            setsockopt(cast(sock_t) socket, IPPROTO_TCP, TCP_NODELAY,
                    cast(char*)&opt, opt.sizeof);
        }();
    }
    ...
    final override void setKeepAlive(StreamSocketFD socket, bool enable) {
        ubyte opt = enable;
        () @trusted{
            setsockopt(cast(sock_t) socket, SOL_SOCKET, SO_KEEPALIVE,
                    cast(char*)&opt, opt.sizeof);
        }();
    }

    final override void read(StreamSocketFD socket, ubyte[] buffer, IOMode mode,
            IOCallback on_read_finish) {
        /*if (buffer.length == 0) {
        ...
        sizediff_t ret;
        () @trusted{
            ret = .recv(cast(sock_t) socket, buffer.ptr, min(buffer.length, int.max), 0);
        }();
    ...
    override void cancelRead(StreamSocketFD socket) {
        assert(m_loop.m_fds[socket].streamSocket.readCallback !is null,
                "Cancelling read when there is no read in progress.");
    ...
    private void onSocketRead(FD fd) {
        ...
        sizediff_t ret = 0;
        () @trusted{
            ret = .recv(cast(sock_t) socket, slot.readBuffer.ptr,
                    min(slot.readBuffer.length, int.max), 0);
        }();
        if (ret < 0) {
            auto err = getSocketError();
            if (!err.among!(EAGAIN, EINPROGRESS)) {
    ...
    final override void write(StreamSocketFD socket, const(ubyte)[] buffer,
            IOMode mode, IOCallback on_write_finish) {
        if (buffer.length == 0) {
        ...
        sizediff_t ret;
        () @trusted{
            ret = .send(cast(sock_t) socket, buffer.ptr, min(buffer.length, int.max), 0);
        }();
    ...
    override void cancelWrite(StreamSocketFD socket) {
        assert(m_loop.m_fds[socket].streamSocket.writeCallback !is null,
                "Cancelling write when there is no write in progress.");
    ...
    private void onSocketWrite(FD fd) {
        ...
        sizediff_t ret;
        () @trusted{
            ret = .send(cast(sock_t) socket, slot.writeBuffer.ptr,
                    min(slot.writeBuffer.length, int.max), 0);
        }();
    ...
    private void onSocketDataAvailable(FD fd) {
        ...
        if (ret < 0) {
            ...
            finalize(ret ? IOStatus.ok : IOStatus.disconnected);
    }
    ...
    final override void shutdown(StreamSocketFD socket, bool shut_read, bool shut_write) {
        ...
        () @trusted{
            .shutdown(cast(sock_t) socket, shut_read ? shut_write ? SHUT_RDWR
                    : SHUT_RD:
                    shut_write ? SHUT_WR : 0);
        }();
        if (st == ConnectionState.passiveClose)
            shut_read = true;
        if (st == ConnectionState.activeClose)
            shut_write = true;
        m_loop.m_fds[socket].streamSocket.state = shut_read ? shut_write ? ConnectionState.closed
            : ConnectionState.passiveClose : shut_write
            ? ConnectionState.activeClose : ConnectionState.connected;
    }
    ...
    package DatagramSocketFD createDatagramSocketInternal(scope Address bind_address,
            scope Address target_address, bool is_internal = true) {
        ...
        if (bind_address && ()@trusted{
                return bind(sockfd, bind_address.name, bind_address.nameLen);
            }() != 0) {
            closeSocket(sockfd);
        ...
        if (target_address) {
            ...
            if (target_address is bind_address) {
                ...
                if (()@trusted{
                        return getsockname(sockfd, cast(sockaddr*)&sa, &addr_len);
                    }() != 0) {
                    closeSocket(sockfd);
                    return DatagramSocketFD.init;
                }
                ...
                ret = () @trusted{
                    return connect(sockfd, cast(sockaddr*)&sa, addr_len);
                }();
            } else
                ret = () @trusted{
                return connect(sockfd, target_address.name, target_address.nameLen);
            }();
    ...
    package DatagramSocketFD adoptDatagramSocketInternal(int socket,
            bool is_internal = true, bool close_on_exec = false) {
    ...
    final override void setTargetAddress(DatagramSocketFD socket, scope Address target_address) {
        () @trusted{
            connect(cast(sock_t) socket, target_address.name, target_address.nameLen);
        }();
    }
    ...
    final override bool setBroadcast(DatagramSocketFD socket, bool enable) {
        ...
        return () @trusted{
            return setsockopt(cast(sock_t) socket, SOL_SOCKET, SO_BROADCAST,
                    &tmp_broad, tmp_broad.sizeof);
        }() == 0;
    }
    ...
    final override bool joinMulticastGroup(DatagramSocketFD socket,
        ...
        default:
            assert(false, "Multicast only supported for IPv4/IPv6 sockets.");
        case AddressFamily.INET:
            ...
            return () @trusted{
                return setsockopt(cast(sock_t) socket, IPPROTO_IP,
    ...
    void receive(DatagramSocketFD socket, ubyte[] buffer, IOMode mode,
            DatagramIOCallback on_receive_finish) @trusted { // DMD 2.072.0-b2: scope considered unsafe
        import std.typecons : scoped;

        assert(mode != IOMode.all,
                "Only IOMode.immediate and IOMode.once allowed for datagram sockets.");

        sizediff_t ret;
        sockaddr_storage src_addr;
        socklen_t src_addr_len = src_addr.sizeof;
        () @trusted{
            ret = .recvfrom(cast(sock_t) socket, buffer.ptr, min(buffer.length,
                    int.max), 0, cast(sockaddr*)&src_addr, &src_addr_len);
        }();
        ...
        scope src_addrc = new RefAddress(() @trusted{
            return cast(sockaddr*)&src_addr;
        }(), src_addr_len);
        on_receive_finish(socket, IOStatus.ok, ret, src_addrc);
    }

    void cancelReceive(DatagramSocketFD socket) {
        assert(m_loop.m_fds[socket].datagramSocket.readCallback !is null,
                "Cancelling read when there is no read in progress.");
        m_loop.setNotifyCallback!(EventType.read)(socket, null);
        m_loop.m_fds[socket].datagramSocket.readBuffer = null;
    }

    private void onDgramRead(FD fd) @trusted { // DMD 2.072.0-b2: scope considered unsafe
        ...
        socklen_t src_addr_len = src_addr.sizeof;
        () @trusted{
            ret = .recvfrom(cast(sock_t) socket, slot.readBuffer.ptr,
                    min(slot.readBuffer.length, int.max), 0,
                    cast(sockaddr*)&src_addr, &src_addr_len);
        }();
        ...
        () @trusted{ return cast(DatagramIOCallback) slot.readCallback; }()(socket,
                IOStatus.ok, ret, src_addrc);
    }

    void send(DatagramSocketFD socket, const(ubyte)[] buffer, IOMode mode,
            Address target_address, DatagramIOCallback on_send_finish) {
        assert(mode != IOMode.all,
                "Only IOMode.immediate and IOMode.once allowed for datagram sockets.");
        ...
        if (target_address) {
            () @trusted{
                ret = .sendto(cast(sock_t) socket, buffer.ptr, min(buffer.length,
                        int.max), 0, target_address.name, target_address.nameLen);
            }();
    ...
    void cancelSend(DatagramSocketFD socket) {
        assert(m_loop.m_fds[socket].datagramSocket.writeCallback !is null,
                "Cancelling write when there is no write in progress.");
        m_loop.setNotifyCallback!(EventType.write)(socket, null);
    ...
    private void onDgramWrite(FD fd) {
        ...
        if (slot.targetAddr) {
            () @trusted{
                ret = .sendto(cast(sock_t) socket, slot.writeBuffer.ptr,
                        min(slot.writeBuffer.length, int.max), 0,
                        slot.targetAddr.name, slot.targetAddr.nameLen);
            }();
        } else {
            () @trusted{
                ret = .send(cast(sock_t) socket, slot.writeBuffer.ptr,
        ...
        if (ret < 0) {
            ...
            if (!err.among!(EAGAIN, EINPROGRESS)) {
                auto l = lockHandle(socket);
                m_loop.setNotifyCallback!(EventType.write)(socket, null);
                () @trusted{ return cast(DatagramIOCallback) slot.writeCallback; }()(socket,
                        IOStatus.error, 0, null);
                return;
            }
        }
    ...
    final override bool releaseRef(SocketFD fd) {
        ...
        if (--slot.common.refCount == base_refcount) {
            ...
            default:
                assert(false, "File descriptor slot is not a socket.");
            case streamSocket:
    ...
    final override bool setOption(DatagramSocketFD socket, DatagramSocketOption option, bool enable) {
        ...
        int tmp = enable;
        return () @trusted{
            return setsockopt(cast(sock_t) socket, proto, opt, &tmp, tmp.sizeof);
        }() == 0;
    }
    ...
    private sock_t createSocket(AddressFamily family, int type) {
        ...
        version (linux) {
            () @trusted{
                sock = socket(family, type | SOCK_NONBLOCK | SOCK_CLOEXEC, 0);
            }();
            if (sock == -1)
                return -1;
        } else {
...
private void closeSocket(sock_t sockfd) @nogc nothrow {
    version (Windows)
        () @trusted{ closesocket(sockfd); }();
    else
        close(sockfd);
}

private void setSocketNonBlocking(SocketFD sockfd, bool close_on_exec = false) @nogc nothrow {
    version (Windows) {
        uint enable = 1;
...
private int getSocketError() @nogc nothrow {
    version (Windows)
