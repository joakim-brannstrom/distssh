...
@safe: /*@nogc:*/ nothrow:
    ...
    override size_t waiterCount() {
        return m_waiterCount + m_timers.pendingCount;
    }

    package void addWaiter() {
        m_waiterCount++;
    }
    ...
    override void runInOwnerThread(ThreadCallback del, intptr_t param) shared {
    ...
    package void* rawUserDataImpl(HANDLE handle, size_t size,
        ...
        if (size > HandleSlot.userData.length)
            assert(false);
    ...
    private bool doProcessEvents(Duration max_wait) {
        ...
        if (max_wait > 0.seconds) {
            DWORD timeout_msecs = max_wait == Duration.max ? INFINITE
                : cast(DWORD) min(max_wait.total!"msecs", DWORD.max);
            auto ret = () @trusted{
                return MsgWaitForMultipleObjectsEx(cast(DWORD) m_registeredEvents.length, m_registeredEvents.ptr,
        ...
        while (()@trusted{ return PeekMessageW(&msg, null, 0, 0, PM_REMOVE); }()) {
            ...
            () @trusted{ TranslateMessage(&msg); DispatchMessageW(&msg); }();
...
private long currStdTime() @safe nothrow {
...
@safe nothrow:
    ...
    @property ref FileSlot file() {
        return specific.get!FileSlot;
    }

    @property ref WatcherSlot watcher() {
        return specific.get!WatcherSlot;
    }
...
package struct FileSlot {
    static struct Direction(bool RO) {
        ...
        void invokeCallback(IOStatus status, size_t bytes_transferred) @safe nothrow {
...
package extern (System) @system nothrow void overlappedIOHandler(alias process, EXTRA...)(
        DWORD error, DWORD bytes_transferred, OVERLAPPED* _overlapped, EXTRA extra) {
