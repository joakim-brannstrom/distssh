...
@safe: /*@nogc:*/ nothrow:
    ...
    DNSLookupID lookupHost(string name, DNSLookupCallback on_lookup_finished) {
        ...
        foreach (af; addrfamilies) {
            ...
            auto ret = () @trusted{
                return WSAStringToAddressW(namew, af, null, cast(sockaddr*)&sa, &addrlen);
            }();
            if (ret != 0)
                continue;
        ...
        version (none) { // Windows 8+
            ...
            enforce(GetAddrInfoExW(namew, null, NS_DNS, null, &addr_hint, &addr_ret,
                    null, &overlapped, &onDnsResult, null) == 0, "Failed to lookup host");
            ...
            case AF_INET:
                addr.sockAddrInet4 = *cast(sockaddr_in*) addr_ret.ai_addr;
                break;
            case AF_INET6:
                addr.sockAddrInet6 = *cast(sockaddr_in6*) addr_ret.ai_addr;
                break;
            }
            ...
            ADDRINFOW* results;
            if (auto ret = ()@trusted{
                    return GetAddrInfoW(namew, null, null, &results);
                }()) {
            ...
            () @trusted{
                typeof(scoped!RefAddress())[16] addr_storage = [
                    ...
                    scoped!RefAddress(), scoped!RefAddress(), scoped!RefAddress(),
                    scoped!RefAddress(), scoped!RefAddress(), scoped!RefAddress(),
                    scoped!RefAddress(), scoped!RefAddress(),
                    scoped!RefAddress(), scoped!RefAddress()
                ];
                RefAddress[16] buf;
                size_t naddr = 0;
