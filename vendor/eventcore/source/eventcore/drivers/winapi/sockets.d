...
final class WinAPIEventDriverSockets : EventDriverSockets {
    @safe :  /*@nogc:*/ nothrow : private {
        alias SocketVector = AlgebraicChoppedVector!(SocketSlot,
                StreamSocketSlot, StreamListenSocketSlot, DatagramSocketSlot);
    ...
    this(WinAPIEventDriverCore core) @trusted {
        m_tid = GetCurrentThreadId();
        m_core = core;

        // setup socket event message window
        setupWindowClass();
        m_hwnd = CreateWindowA("VibeWin32MessageWindow", "VibeWin32MessageWindow",
                0, 0, 0, 0, 0, HWND_MESSAGE, null, null, null);
        SetWindowLongPtrA(m_hwnd, GWLP_USERDATA, cast(ULONG_PTR) cast(void * ) this);
        assert(cast(WinAPIEventDriverSockets) cast(void * ) GetWindowLongPtrA(m_hwnd,
                GWLP_USERDATA) is this);
    }

    package @property size_t waiterCount() const {
        return m_waiters;
    }

    override StreamSocketFD connectStream(scope Address peer_address,
            scope Address bind_address, ConnectCallback on_connect) @trusted {
        assert(m_tid == GetCurrentThreadId());

        auto fd = WSASocketW(peer_address.addressFamily, SOCK_STREAM,
        ...
        void invalidateSocket() @nogc @trusted nothrow {
            closesocket(fd);
            fd = INVALID_SOCKET;
        }
    ...
    private StreamSocketFD adoptStreamInternal(SOCKET socket, ConnectionState state) {
        ...
        () @trusted {
            WSAAsyncSelect(socket, m_hwnd, WM_USER_SOCKET, FD_CONNECT | FD_CLOSE);
        }
        ();
    ...
    override StreamListenSocketFD listenStream(scope Address bind_address,
            StreamListenOptions options, AcceptCallback on_accept) {
        auto fd = () @trusted {
            return WSASocketW(bind_address.addressFamily, SOCK_STREAM,
                    IPPROTO_TCP, null, 0, WSA_FLAG_OVERLAPPED);
        }
        ();
        if (fd == INVALID_SOCKET)
    ...
    override void waitForConnections(StreamListenSocketFD sock, AcceptCallback on_accept) {
        ...
        () @trusted {
            WSAAsyncSelect(sock, m_hwnd, WM_USER_SOCKET, FD_ACCEPT);
        }
        ();
        m_core.addWaiter();
    }
    ...
    override bool getLocalAddress(SocketFD sock, scope RefAddress dst) {
        ...
        if (()@trusted {
                return getsockname(sock, dst.name,  & addr_len); }
                () != 0)
                return false;
                dst.cap(addr_len);
                return true;
            ...
            override bool getRemoteAddress(SocketFD sock, scope RefAddress dst) {
                ...
                socklen_t addr_len = dst.nameLen;
                if (()@trusted {
                        return getpeername(sock, dst.name,  & addr_len); }
                        () != 0)
                        return false;
                        dst.cap(addr_len);
                        return true;
                    ...
                    override void setTCPNoDelay(StreamSocketFD socket, bool enable) @trusted {
                        BOOL eni = enable;
                    ...
                    override void read(StreamSocketFD socket, ubyte[] buffer,
                    IOMode mode, IOCallback on_read_finish) {
                        auto slot = () @trusted {
                            return  & m_sockets[socket].streamSocket();
                        }
                        ();
                        slot.read.buffer = buffer;
                        slot.read.mode = mode;
                        slot.read.wsabuf[0].len = buffer.length;
                        slot.read.wsabuf[0].buf = () @trusted {
                            return buffer.ptr;
                        }
                        ();

                        auto ovl = mode == IOMode.immediate ? null
                        :  & slot.read.overlapped.overlapped;
                        DWORD flags = 0;
                        auto handler =  & overlappedIOHandler!(onIOReadCompleted, DWORD);
                        auto ret = () @trusted {
                            return WSARecv(socket,  & slot.read.wsabuf[0],
                            slot.read.wsabuf.length, null,  & flags, ovl, handler);
                        }
                        ();
                        if (ret == SOCKET_ERROR) {
                            auto err = WSAGetLastError();
                    ...
                    private static nothrow void onIOReadCompleted(DWORD dwError,
                    DWORD cbTransferred, OVERLAPPED_CORE * lpOverlapped) {
                        auto slot = () @trusted {
                            return cast(SocketVector.FullField * ) lpOverlapped.hEvent;
                        }
                        ();
                        ...
                        void invokeCallback(IOStatus status, size_t nsent) @safe nothrow {
                        ...
                        slot.streamSocket.read.buffer
                        = slot.streamSocket.read.buffer[cbTransferred .. $];
                        ...
                        if (slot.streamSocket.read.mode == IOMode.once
                            || !slot.streamSocket.read.buffer.length) {
                            invokeCallback(IOStatus.ok, cbTransferred);
                            return;
                        }

                        slot.streamSocket.read.wsabuf[0].len = slot.streamSocket.read.buffer.length;
                        slot.streamSocket.read.wsabuf[0].buf = () @trusted {
                            return cast(ubyte * ) slot.streamSocket.read.buffer.ptr;
                        }
                        ();
                        auto ovl = slot.streamSocket.read.mode == IOMode.immediate
                        ? null :  & slot.streamSocket.read.overlapped.overlapped;
                        DWORD flags = 0;
                        auto handler =  & overlappedIOHandler!(onIOReadCompleted, DWORD);
                        auto ret = () @trusted {
                            return WSARecv(slot.common.fd,  & slot.streamSocket.read.wsabuf[0],
                            slot.streamSocket.read.wsabuf.length, null,  & flags, ovl, handler);
                        }
                        ();
                        if (ret == SOCKET_ERROR) {
                            auto err = WSAGetLastError();
                    ...
                    override void write(StreamSocketFD socket,
                    const(ubyte)[] buffer, IOMode mode, IOCallback on_write_finish) {
                        auto slot = () @trusted {
                            return  & m_sockets[socket].streamSocket();
                        }
                        ();
                        ...
                        slot.write.wsabuf[0].buf = () @trusted {
                            return cast(ubyte * ) buffer.ptr;
                        }
                        ();

                        auto ovl = mode == IOMode.immediate ? null
                        :  & m_sockets[socket].streamSocket.write.overlapped.overlapped;
                        auto handler =  & overlappedIOHandler!(onIOWriteCompleted, DWORD);
                        auto ret = () @trusted {
                            return WSASend(socket,  & slot.write.wsabuf[0],
                            slot.write.wsabuf.length, null, 0, ovl, handler);
                        }
                        ();
                        if (ret == SOCKET_ERROR) {
                            auto err = WSAGetLastError();
                    ...
                    private static nothrow void onIOWriteCompleted(DWORD dwError,
                    DWORD cbTransferred, OVERLAPPED_CORE * lpOverlapped) {
                        auto slot = () @trusted {
                            return cast(SocketVector.FullField * ) lpOverlapped.hEvent;
                        }
                        ...
                        slot.streamSocket.write.buffer
                        = slot.streamSocket.write.buffer[cbTransferred .. $];
                        ...
                        if (slot.streamSocket.write.mode == IOMode.once
                            || !slot.streamSocket.write.buffer.length) {
                            invokeCallback(IOStatus.ok, cbTransferred);
                            return;
                        }

                        slot.streamSocket.write.wsabuf[0].len
                        = slot.streamSocket.write.buffer.length;
                        slot.streamSocket.write.wsabuf[0].buf = () @trusted {
                            return cast(ubyte * ) slot.streamSocket.write.buffer.ptr;
                        }
                        ();
                        auto ovl = slot.streamSocket.write.mode == IOMode.immediate
                        ? null :  & slot.streamSocket.write.overlapped.overlapped;
                        auto handler =  & overlappedIOHandler!(onIOWriteCompleted, DWORD);
                        auto ret = () @trusted {
                            return WSASend(slot.common.fd,  & slot.streamSocket.write.wsabuf[0],
                            slot.streamSocket.write.wsabuf.length, null, 0, ovl, handler);
                        }
                        ();
                        if (ret == SOCKET_ERROR) {
                            auto err = WSAGetLastError();
                    ...
                    override void shutdown(StreamSocketFD socket,
                    bool shut_read = true, bool shut_write = true) {
                        () @trusted {
                    ...
                    override void cancelRead(StreamSocketFD socket) @trusted {
                        if (!m_sockets[socket].streamSocket.read.callback)
                            return;
                        CancelIoEx(cast(HANDLE) cast(SOCKET) socket,
                        cast(LPOVERLAPPED) & m_sockets[socket].streamSocket.read.overlapped);
                        m_sockets[socket].streamSocket.read.callback = null;
                        m_core.removeWaiter();
                    }

                    override void cancelWrite(StreamSocketFD socket) @trusted {
                        if (!m_sockets[socket].streamSocket.write.callback)
                            return;
                        CancelIoEx(cast(HANDLE) cast(SOCKET) socket,
                        cast(LPOVERLAPPED) & m_sockets[socket].streamSocket.write.overlapped);
                        m_sockets[socket].streamSocket.write.callback = null;
                        m_core.removeWaiter();
                    }

                    final override DatagramSocketFD createDatagramSocket(
                    scope Address bind_address, scope Address target_address) {
                        auto fd = () @trusted {
                            return WSASocketW(bind_address.addressFamily,
                    ...
                    final override void setTargetAddress(DatagramSocketFD socket,
                    scope Address target_address) {
                        () @trusted {
                            connect(cast(SOCKET) socket, target_address.name,
                            target_address.nameLen);
                        }
                        ();
                    }
                    ...
                    final override bool setBroadcast(DatagramSocketFD socket, bool enable) {
                        ...
                        return () @trusted {
                            return setsockopt(cast(SOCKET) socket, SOL_SOCKET,
                            SO_BROADCAST,  & tmp_broad, tmp_broad.sizeof);
                        }
                        () == 0;
                    }

                    final override bool joinMulticastGroup(DatagramSocketFD socket,
                    scope Address multicast_address, uint interface_index = 0) {
                        import std.socket : AddressFamily;

                        switch (multicast_address.addressFamily) {
                        default : assert(false, "Multicast only supported for IPv4/IPv6 sockets.");
                        case AddressFamily.INET : struct ip_mreq {
                                in_addr imr_multiaddr; /* IP multicast address of group */
                                in_addr imr_interface; /* local IP address of interface */
                            }
                            auto addr = () @trusted {
                                return cast(sockaddr_in * ) multicast_address.name;
                            }
                            ();
                            ip_mreq mreq;
                            ...
                            return () @trusted {
                                return setsockopt(cast(SOCKET) socket, IPPROTO_IP,
                                IP_ADD_MEMBERSHIP,  & mreq, ip_mreq.sizeof);
                            }
                            () == 0;
                        case AddressFamily.INET6 : struct ipv6_mreq {
                                in6_addr ipv6mr_multiaddr;
                                uint ipv6mr_interface;
                            }
                            auto addr = () @trusted {
                                return cast(sockaddr_in6 * ) multicast_address.name;
                            }
                            ();
                            ipv6_mreq mreq;
                            ...
                            return () @trusted {
                                return setsockopt(cast(SOCKET) socket, IPPROTO_IP,
                                IPV6_JOIN_GROUP,  & mreq, ipv6_mreq.sizeof);
                            }
                            () == 0;
                        }
                    }

                    override void receive(DatagramSocketFD socket, ubyte[] buffer,
                    IOMode mode, DatagramIOCallback on_read_finish) {
                        auto slot = () @trusted {
                            return  & m_sockets[socket].datagramSocket();
                        }
                        ();
                        slot.read.buffer = buffer;
                        slot.read.wsabuf[0].buf = () @trusted {
                        ...
                        auto ret = () @trusted {
                            return WSARecvFrom(socket,  & slot.read.wsabuf[0],
                            slot.read.wsabuf.length, null,  & flags,
                            cast(SOCKADDR * ) & slot.sourceAddr,  & slot.sourceAddrLen,
                            ovl, handler);
                        }
                        ();
                        if (ret == SOCKET_ERROR) {
                        ...
                        if (mode == IOMode.immediate)
                            () @trusted {
                            CancelIoEx(cast(HANDLE) cast(SOCKET) socket,
                            cast(LPOVERLAPPED) & slot.read.overlapped);
                        }
                        ();
                    ...
                    override void cancelReceive(DatagramSocketFD socket) @trusted {
                        if (!m_sockets[socket].datagramSocket.read.callback)
                            return;
                        CancelIoEx(cast(HANDLE) cast(SOCKET) socket,
                        cast(LPOVERLAPPED) & m_sockets[socket].datagramSocket.read.overlapped);
                        m_sockets[socket].datagramSocket.read.callback = null;
                        m_core.removeWaiter();
                    }

                    private static nothrow void onIOReceiveCompleted(DWORD dwError,
                    DWORD cbTransferred, OVERLAPPED_CORE * lpOverlapped) {
                        auto slot = () @trusted {
                            return cast(SocketVector.FullField * ) lpOverlapped.hEvent;
                        }
                        ...
                        void invokeCallback(IOStatus status, size_t nsent) @safe nothrow {
                            ...
                            scope addr = new RefAddress(cast(sockaddr * ) & slot.datagramSocket.sourceAddr,
                            slot.datagramSocket.sourceAddrLen);
                            if (slot.common.driver.releaseRef(cast(DatagramSocketFD) slot.common.fd))
                                cb(cast(DatagramSocketFD) slot.common.fd, status,
                                nsent, status == IOStatus.ok ? addr : null);
                        }

                        slot.datagramSocket.read.bytesTransferred += cbTransferred;
                        slot.datagramSocket.read.buffer
                        = slot.datagramSocket.read.buffer[cbTransferred .. $];

                        if (!dwError && (slot.datagramSocket.read.mode != IOMode.all
                            || !slot.datagramSocket.read.buffer.length)) {
                            invokeCallback(IOStatus.ok, cbTransferred);
                            return;
                        }

                        if (dwError == WSA_OPERATION_ABORTED
                            && slot.datagramSocket.write.mode == IOMode.immediate) {
                            invokeCallback(IOStatus.wouldBlock, 0);
                            return;
                        }
                        ...
                        slot.datagramSocket.read.wsabuf[0].len
                        = slot.datagramSocket.read.buffer.length;
                        slot.datagramSocket.read.wsabuf[0].buf = () @trusted {
                            return cast(ubyte * ) slot.datagramSocket.read.buffer.ptr;
                        }
                        ();
                        auto ovl = slot.datagramSocket.read.mode == IOMode.immediate
                        ? null :  & slot.datagramSocket.read.overlapped.overlapped;
                        DWORD flags = 0;
                        auto handler =  & overlappedIOHandler!(onIOReceiveCompleted, DWORD);
                        auto ret = () @trusted {
                            return WSARecvFrom(slot.common.fd,  & slot.datagramSocket.read.wsabuf[0],
                            slot.datagramSocket.read.wsabuf.length, null,  & flags,
                            cast(SOCKADDR * ) & slot.datagramSocket.sourceAddr,
                             & slot.datagramSocket.sourceAddrLen, ovl, handler);
                        }
                        ();
                        if (ret == SOCKET_ERROR) {
                            auto err = WSAGetLastError();
                    ...
                    override void send(DatagramSocketFD socket, const(ubyte)[] buffer, IOMode mode,
                    Address target_address, DatagramIOCallback on_write_finish) {
                        auto slot = () @trusted {
                            return  & m_sockets[socket].datagramSocket();
                        }
                        ();
                        ...
                        auto ret = () @trusted {
                            return WSASendTo(socket,  & slot.write.wsabuf[0],
                            slot.write.wsabuf.length, null, 0, tan, tal, ovl, handler);
                        }
                        ();
                        ...
                        if (mode == IOMode.immediate)
                            () @trusted {
                            CancelIoEx(cast(HANDLE) cast(SOCKET) socket,
                            cast(LPOVERLAPPED) & slot.write.overlapped);
                        }
                        ();
                    ...
                    override void cancelSend(DatagramSocketFD socket) @trusted {
                        if (!m_sockets[socket].datagramSocket.write.callback)
                            return;
                        CancelIoEx(cast(HANDLE) cast(SOCKET) socket,
                        cast(LPOVERLAPPED) & m_sockets[socket].datagramSocket.write.overlapped);
                        m_sockets[socket].datagramSocket.write.callback = null;
                        m_core.removeWaiter();
                    }

                    private static nothrow void onIOSendCompleted(DWORD dwError,
                    DWORD cbTransferred, OVERLAPPED_CORE * lpOverlapped) {
                        auto slot = () @trusted {
                            return cast(SocketVector.FullField * ) lpOverlapped.hEvent;
                        }
                        ...
                        slot.datagramSocket.write.buffer
                        = slot.datagramSocket.write.buffer[cbTransferred .. $];

                        if (!dwError && (slot.datagramSocket.write.mode != IOMode.all
                            || !slot.datagramSocket.write.buffer.length)) {
                            invokeCallback(IOStatus.ok, cbTransferred);
                            return;
                        }
                        ...
                        slot.datagramSocket.write.wsabuf[0].len
                        = slot.datagramSocket.write.buffer.length;
                        slot.datagramSocket.write.wsabuf[0].buf = () @trusted {
                            return cast(ubyte * ) slot.datagramSocket.write.buffer.ptr;
                        }
                        ();
                        auto tan = slot.datagramSocket.targetAddr
                        ? slot.datagramSocket.targetAddr.name : null;
                        auto tal = slot.datagramSocket.targetAddr
                        ? slot.datagramSocket.targetAddr.nameLen : 0;
                        auto ovl = slot.datagramSocket.write.mode == IOMode.immediate
                        ? null :  & slot.datagramSocket.write.overlapped.overlapped;
                        auto handler =  & overlappedIOHandler!(onIOSendCompleted, DWORD);
                        auto ret = () @trusted {
                            return WSASendTo(slot.common.fd,  & slot.datagramSocket.write.wsabuf[0],
                            slot.datagramSocket.write.wsabuf.length, null, 0,
                            tan, tal, ovl, handler);
                        }
                        ();
                        if (ret == SOCKET_ERROR) {
                    ...
                    override void addRef(SocketFD fd) {
                        assert(m_sockets[fd].common.refCount > 0,
                        "Adding reference to unreferenced socket FD.");
                    ...
                    override bool releaseRef(SocketFD fd) {
                        ...
                        ();
                        nogc_assert(slot.common.refCount > 0,
                        "Releasing reference to unreferenced socket FD.");
                        if (--slot.common.refCount == 0) {
                            final switch (slot.specific.kind) with (SocketVector.FieldType) {
                            case Kind.none : break;
                            case Kind.streamSocket : cancelRead(cast(StreamSocketFD) fd);
                                cancelWrite(cast(StreamSocketFD) fd);
                                m_core.discardEvents( & slot.streamSocket.read.overlapped,
                                 & slot.streamSocket.write.overlapped);
                                break;
                            case Kind.streamListen
                                : if (m_sockets[fd].streamListen.acceptCallback) m_core.removeWaiter();
                                break;
                            case Kind.datagramSocket : cancelReceive(cast(DatagramSocketFD) fd);
                                cancelSend(cast(DatagramSocketFD) fd);
                                m_core.discardEvents( & slot.datagramSocket.read.overlapped,
                                 & slot.datagramSocket.write.overlapped);
                                break;
                            }
                    ...
                    final override bool setOption(DatagramSocketFD socket,
                    DatagramSocketOption option, bool enable) {
                        int proto, opt;
                        final switch (option) {
                        case DatagramSocketOption.broadcast:
                            proto = SOL_SOCKET;
                            opt = SO_BROADCAST;
                            break;
                        case DatagramSocketOption.multicastLoopback:
                            proto = IPPROTO_IP;
                        ...
                        int tmp = enable;
                        return () @trusted{
                            return setsockopt(cast(SOCKET) socket, proto, opt, &tmp, tmp.sizeof);
                        }() == 0;
                    }

                    final override bool setOption(StreamSocketFD socket,
                    StreamSocketOption option, bool enable) {
                        int proto, opt;
                        final switch (option) {
                        case StreamSocketOption.noDelay:
                            proto = IPPROTO_TCP;
                            opt = TCP_NODELAY;
                            break;
                        case StreamSocketOption.keepAlive:
                            proto = SOL_SOCKET;
                            opt = SO_KEEPALIVE;
                            break;
                        }
                    ...
                    final protected override void* rawUserData(StreamSocketFD descriptor,
                    ...
                    final protected override void* rawUserData(StreamListenSocketFD descriptor,
                    size_t size, DataInitializer initialize, DataInitializer destroy) @system {
                        return rawUserDataImpl(descriptor, size, initialize, destroy);
                    }

                    final protected override void* rawUserData(DatagramSocketFD descriptor,
                    size_t size, DataInitializer initialize, DataInitializer destroy) @system {
                        return rawUserDataImpl(descriptor, size, initialize, destroy);
                    }

                    private void* rawUserDataImpl(FD descriptor, size_t size,
                    DataInitializer initialize, DataInitializer destroy) @system {
                        ...
                        assert(size <= SocketSlot.userData.length,
                        "Requested user data is too large.");
                        if (size > SocketSlot.userData.length)
                            assert(false);
                    ...
                    package void clearSocketSlot(FD fd) {
                        ...
                        if (slot.common.userDataDestructor)
                            () @trusted{
                            slot.common.userDataDestructor(slot.common.userData.ptr);
                        }();
                        *slot = m_sockets.FullField.init;
                    }

                    private static nothrow extern (System) LRESULT onMessage(HWND wnd,
                    UINT msg, WPARAM wparam, LPARAM lparam) {
                        auto driver = () @trusted{
                            return cast(WinAPIEventDriverSockets) cast(void*) GetWindowLongPtrA(wnd,
                            GWLP_USERDATA);
                        }();
                        switch (msg) {
                        default:
			...
			m_sockets[listenfd].streamListen.acceptCallback(cast(StreamListenSocketFD)listenfd, fd, addrc);
							*/
                                    SOCKADDR_STORAGE addr;
                                    socklen_t addr_len = addr.sizeof;
                                    auto clientsockfd = () @trusted{
                                        return WSAAccept(sock, cast(sockaddr*)&addr,
                                        &addr_len, null, 0);
                                    }();
                                    ...
                                    scope RefAddress addrc = new RefAddress(() @trusted{
                                        return cast(sockaddr*)&addr;
                                    }(), addr_len);
                                    slot.streamListen.acceptCallback(cast(StreamListenSocketFD) sock,
                                    clientsock, addrc);
                                }
                                break;
                    ...
                    void setupWindowClass() nothrow @trusted {
                    ...
                    static struct SocketSlot {
                        ...
                        @property inout(WinAPIEventDriverCore) core() @safe nothrow inout {
                            return driver.m_core;
                        }
