...
void testCallback(WatcherID w, in ref FileChange ch) @safe nothrow {
...
void expectChange(FileChange ch, bool expect_change) {
    ...
    again: while (!pendingChanges.length) {
        ...
        switch (er) {
        default:
            assert(false, format("Unexpected event loop exit code: %s", er));
        ...
        case ExitReason.outOfWaiters:
            assert(!expect_change,
                    "No watcher left, but expected change.");
            return;
        }
