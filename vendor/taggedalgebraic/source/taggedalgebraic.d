    ...
    T opCast(T)() {
        ...
        case __traits(getMember, Kind, fieldNames[i]):
                static if (is(typeof(trustedGet!(fieldNames[i])) : T))
                    ...
                    assert(false,
                            "Cannot cast a " ~ m_kind.to!string
...
ref inout(T) get(T, U)(ref inout(TaggedAlgebraic!U) ta) {
    ...
    assert(hasType!(T, U)(ta), () {
        scope (failure)
            assert(false);
        return format("Trying to get %s but have %s.", T.stringof, ta.kind);
    }());
...
private static auto implementOp(OpKind kind, string name, T, ARGS...)(ref T self, auto ref ARGS args) {
    ...
    switch (self.m_kind) {
    default:
        assert(false, "Operator " ~ name ~ " (" ~ kind.stringof
...
private template OpInfo(U, OpKind kind, string name, ARGS...) {
    import std.traits : CopyTypeQualifiers, FieldTypeTuple, FieldNameTuple,
        ReturnType;
