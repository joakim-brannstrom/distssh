...
struct BitmappedBlock(size_t theBlockSize, uint theAlignment = platformAlignment,
    ...
    @system unittest {
        ...
        scope (exit)
            AlignedMallocator.instance.deallocate(m);
        testAllocator!(() => BitmappedBlock(m));
    }

    static assert(theBlockSize > 0 && theAlignment.isGoodStaticAlignment);
    static assert(theBlockSize == chooseAtRuntime || theBlockSize % theAlignment == 0,
    ...
    */
    ...
    private size_t totalAllocation(size_t capacity) {
        ...
        immutable initialAlignment = min(parentAlignment, 1U << trailingZeros(leadingUlongs * 8));
        auto maxSlack = alignment <= initialAlignment ? 0 : alignment - initialAlignment;
        //writeln(maxSlack);
    ...
    */
    this(ubyte[] data) {
        ...
        assert(a >= size_t.alignof || !data.ptr, "Data must be aligned properly");
        ...
        for (; _blocks; --_blocks) {
            ...
            auto payload = data[controlWords * 8 .. $].roundStartToMultipleOf(alignment);
    ...
    */
    void[] alignedAllocate(size_t n, uint a) {
        ...
        auto slackTrailingBlocks = ((b.ptr + b.length) - (result.ptr + result.length)) / blockSize;
    ...
    */
    Ternary owns(void[] b) const {
        ...
        return Ternary(b.ptr >= _payload.ptr && b.ptr + b.length <= _payload.ptr + _payload.length);
    }
    ...
    */
    private Tuple!(size_t, uint) allocateAt(size_t wordIdx, uint msbIdx,
        ...
        if (msbIdx + blocks <= 64) {
            ...
            if (setBitsIfZero(_control.rep[wordIdx], cast(uint)(64 - msbIdx - blocks), 63 - msbIdx)) {
                // Success
                result = blocksFor(wordIdx, msbIdx, blocks);
                return tuple(size_t.max, 0u);
            }
            // Can't allocate, make a suggestion
            return msbIdx + blocks == 64 ? tuple(wordIdx + 1, 0u) : tuple(wordIdx,
                    cast(uint)(msbIdx + blocks));
        }
    ...
    private void[] hugeAlloc(size_t blocks) {
        ...
        return _payload[cast(size_t)(i * blockSize) .. cast(size_t)((i + blocks) * blockSize)];
    }
    ...
    */
    @trusted bool expand(ref void[] b, immutable size_t delta) {
        ...
        */
        if ((b.ptr - _payload.ptr) % blockSize)
            return false;
        ...
        // Try the maximum
        const wordIdx = blockIdxAfter / 64, msbIdx = cast(uint)(blockIdxAfter % 64);
        void[] p;
        ...
        assert(p.ptr == b.ptr + blocksOld * blockSize, text(p.ptr, " != ",
                b.ptr + blocksOld * blockSize));
        b = b.ptr[0 .. b.length + delta];
        return true;
    }
    ...
    */
    bool deallocate(void[] b) {
        ...
        auto begin = _payload.ptr + blockIdx * blockSize, end = b.ptr + b.length;
        b = begin[0 .. end - begin];
        ...
        if (_startIdx > wordIdx)
            _startIdx = wordIdx;

        // Three stages: heading bits, full words, leftover bits
        if (msbIdx) {
            if (blocks + msbIdx <= 64) {
                resetBits(_control.rep[wordIdx], cast(uint)(64 - msbIdx - blocks), 63 - msbIdx);
                return true;
            } else {
    ...
    */
    ...
    void dump() {
        ...
        writefln("  _startIdx=%s; blockSize=%s; blocks=%s", _startIdx, blockSize, _blocks);
        if (!_control.length)
            return;
        uint blockCount = 1;
        ...
        for (size_t i = 1;; ++i) {
            if (i >= _blocks || _control[i] != inAllocatedStore) {
                writefln("  %s block at 0x%s, length: %s (%s*%s)", inAllocatedStore ? "Busy" : "Free",
                        cast(void*) start, blockCount * blockSize, blockCount, blockSize);
                if (i >= _blocks)
                    break;
...
@system unittest {
    static void testAllocateAll(size_t bs)(uint blocks, uint blocksAtATime) {
        ...
        begin: foreach (i; 0 .. blocks / blocksAtATime) {
        ...
        if (blocks >= blocksAtATime) {
            foreach (i; 0 .. blocks / blocksAtATime - 1) {
                ...
                assert(b.length == bs * blocksAtATime, text(i, ": ", b.length));
                (cast(ubyte[]) b)[] = 0xff;
                a.expand(b, blocksAtATime * bs) || assert(0, text(i));
                (cast(ubyte[]) b)[] = 0xfe;
...
*/
struct BitmappedBlockWithInternalPointers(size_t theBlockSize,
        uint theAlignment = platformAlignment, ParentAllocator = NullAllocator) {
    import std.conv : text;
    import stdx.allocator.internal : Ternary;
    ...
    @system unittest {
        ...
        auto m = cast(ubyte[])(AlignedMallocator.instance.alignedAllocate(1024 * 64, theAlignment));
        scope (exit)
            AlignedMallocator.instance.deallocate(m);
    ...
    */
    ...
    void[] allocate(size_t bytes) {
        ...
        immutable blocks = (r.length + _heap.blockSize - 1) / _heap.blockSize;
        ...
        assert(block + blocks == _allocStart.length || _allocStart[block + blocks] == 1);
        return r;
    }
    ...
    bool expand(ref void[] b, size_t bytes) {
        ...
        if (b is null)
            return false;
        immutable oldBlocks = (b.length + _heap.blockSize - 1) / _heap.blockSize;
        assert(oldBlocks);
        immutable newBlocks = (b.length + bytes + _heap.blockSize - 1) / _heap.blockSize;
        assert(newBlocks >= oldBlocks);
        immutable block = (b.ptr - _heap._payload.ptr) / _heap.blockSize;
        assert(_allocStart[block]);
        if (!ensureRoomForAllocStart(block + newBlocks) || !_heap.expand(b, bytes)) {
            return false;
        }
    ...
    Ternary resolveInternalPointer(const void* p, ref void[] result) {
        if (p < _heap._payload.ptr || p >= _heap._payload.ptr + _heap._payload.length) {
            return Ternary.no;
        }
        // Find block start
        auto block = (p - _heap._payload.ptr) / _heap.blockSize;
        if (block >= _allocStart.length)
            return Ternary.no;
        ...
        result = _heap._payload.ptr[cast(size_t)(_heap.blockSize * i) .. cast(
                size_t)(_heap.blockSize * j)];
        return Ternary.yes;
    }
...
private struct BitVector {
    ...
    void opSliceAssign(bool b, ulong x, ulong y) {
        ...
        if (i1 == i2) {
            ...
            assert(b1 >= b2);
            if (b)
                setBits(_rep[i1], b2, b1);
            else
                resetBits(_rep[i1], b2, b1);
        } else {
            // Spans multiple words
            assert(i1 < i2);
            if (b)
                setBits(_rep[i1], 0, b1);
            else
                resetBits(_rep[i1], 0, b1);
            _rep[i1 + 1 .. i2] = b;
            if (b)
                setBits(_rep[i2], b2, 63);
            else
                resetBits(_rep[i2], b2, 63);
        }
    }
