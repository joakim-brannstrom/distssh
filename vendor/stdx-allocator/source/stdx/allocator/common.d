...
*/
@safe @nogc nothrow pure package size_t roundUpToMultipleOf(size_t s, uint base) {
    assert(base);
    auto rem = s % base;
...
*/
@safe @nogc nothrow pure package size_t roundUpToAlignment(size_t n, uint alignment) {
...
*/
@safe @nogc nothrow pure package size_t divideRoundUp(size_t a, size_t b) {
    assert(b);
    return (a + b - 1) / b;
}
...
Returns `s` rounded up to a multiple of `base`.
*/
@nogc nothrow pure package void[] roundStartToMultipleOf(void[] s, uint base) {
    assert(base);
...
*/
@safe @nogc nothrow pure package size_t roundUpToPowerOf2(size_t s) {
...
*/
@safe @nogc nothrow pure package uint trailingZeros(ulong x) {
    uint result;
...
*/
@nogc nothrow pure package bool alignedAt(T)(T* ptr, uint alignment) {
...
a divisor of `ptr`.
*/
@nogc nothrow pure package uint effectiveAlignment(void* ptr) {
...
*/
@nogc nothrow pure package void* alignDownTo(void* ptr, uint alignment) {
    import stdx.allocator.internal : isPowerOf2;
...
@safe @nogc nothrow pure package bool isGoodStaticAlignment(uint x) {
    import stdx.allocator.internal : isPowerOf2;

    return x.isPowerOf2;
}

@safe @nogc nothrow pure package bool isGoodDynamicAlignment(uint x) {
    import stdx.allocator.internal : isPowerOf2;
...
bool reallocate(Allocator)(ref Allocator a, ref void[] b, size_t s) {
    ...
    if (newB.length != s)
        return false;
    if (newB.length <= b.length)
        newB[] = b[0 .. newB.length];
    else
        newB[0 .. b.length] = b[];
...
bool alignedReallocate(Allocator)(ref Allocator alloc, ref void[] b, size_t s, uint a) {
    static if (hasMember!(Allocator, "expand")) {
        if (b.length <= s && b.ptr.alignedAt(a) && alloc.expand(b, s - b.length))
            return true;
    } else {
        if (b.length == s)
            return true;
...
string forwardToMember(string member, string[] funs...) {
    ...
    auto ref " ~ fun
            ~ "(Parameters!(typeof(" ~ member ~ "." ~ fun ~ ")) args)
    {
        return "
            ~ member ~ "." ~ fun ~ "(args);
    }\n";
...
version (unittest) {
    ...
    package void testAllocator(alias make)() {
        ...
        alias A = typeof(make());
        scope (failure)
            stderr.writeln("testAllocator failed for ", A.stringof);
        ...
        // Test goodAllocSize
        assert(a.goodAllocSize(1) >= A.alignment, text(a.goodAllocSize(1), " < ", A.alignment));
    ...
    package void testAllocatorObject(AllocInterface)(AllocInterface a)
            if (is(AllocInterface : IAllocator) || is(AllocInterface : shared ISharedAllocator)) {
        import std.conv : text;
        import stdx.allocator.internal : isPowerOf2;
        import std.stdio : writeln, stderr;
        import stdx.allocator.internal : Ternary;
        ...
        scope (failure)
            stderr.writeln("testAllocatorObject failed for ", AllocInterface.stringof);
