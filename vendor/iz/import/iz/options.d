...
enum ArgFlag: ubyte
{
    ...
    allowShort,
    /// Indicates that the argument is for getting help, which has for effect to cancel handling of any other arguments.
    isHelp
}
...
struct Argument
{
    ...
    /// ditto
    bool isHelp() const {return ArgFlag.isHelp in flags;}
}
...
unittest
{
    ...
    assert(!Argument("--help", "", ArgFlags(ArgFlag.allowShort)).isHelp);
}
...
if (is(typeof(mustThrow) : bool))
{
    ...
    alias unsortedDescr = ArgDescriptions!Locations;
    alias unsortedTrgts = ArgumentTargets!Locations;
    enum cmp(Argument a) = a.isHelp;
    enum hIndex = iz.sugar.firstStaticIndexOf!(cmp, unsortedDescr);
    static if (hIndex != -1)
    {
        alias firstDescr = AliasSeq!(unsortedDescr[hIndex]);
        alias firstTrgts = AliasSeq!(unsortedTrgts[hIndex]);
        alias tailDescr = AliasSeq!(unsortedDescr[0 .. hIndex], unsortedDescr[hIndex + 1 .. $]);
        alias tailTrgts = AliasSeq!(unsortedTrgts[0 .. hIndex], unsortedTrgts[hIndex + 1 .. $]);
    }
    else
    {
        alias firstDescr = AliasSeq!();
        alias firstTrgts = AliasSeq!();
        alias tailDescr = unsortedDescr;
        alias tailTrgts = unsortedTrgts;
    }


    bool             result = true;
    alias            argTrgts = AliasSeq!(firstTrgts, tailTrgts);
    static immutable argDescr = AliasSeq!(firstDescr, tailDescr);
    string[2]        argParts;
        ...
        foreach(ref arg; args)
        {
            ...
            if (shortOption || argParts[0] == argDescr[i].name)
            {
                ...
                else
                {
                    ...
                    {
                        if (argParts[1].length == 0 || argParts[1].among("true", "+"))
                        {
                            done[i] = true;
                            mixin(t[1]) = true;
                            static if (argDescr[i].isStopper)
                        ...
                        else if (argParts[1].among("false", "-"))
                        {
                            done[i] = true;
                            mixin(t[1]) = false;
                            static if (argDescr[i].isStopper)
...
@safe nothrow unittest
{
    ...
    assert(Container.a);
    assert(handleArguments!(CantThrow, Container)(["-a-"]));
    assert(!Container.a);
    assert(handleArguments!(CantThrow, Container)(["-a+"]));
    assert(Container.a);
}
...
version(unittest) private static struct ContainerChars
{
    static @Argument("-c", "") char c;
    static @Argument("-d", "") dchar d;
}

@safe unittest
{
    assert(!handleArguments!(CantThrow, ContainerChars)([`-cé`]));
    assert(handleArguments!(CantThrow, ContainerChars)([`-dé`]));
}
...
version(unittest) private static struct ContainerForHelp
{
    static bool t;
    static @Argument("-c", "", ArgFlags(AFmandatory)) char x = 'E';
    static @Argument("-h", "", ArgFlags(AFisHelp)) void h() @safe {t = !t;}
}

@safe unittest
{
    assert(handleArguments!(CanThrow, ContainerForHelp)([`-x0`, `-h`]));
    assert(ContainerForHelp.x == 'E');
    assert(ContainerForHelp.t);
    assert(handleArguments!(CanThrow, ContainerForHelp)([`-h`, `-x0`]));
    assert(ContainerForHelp.x == 'E');
    assert(!ContainerForHelp.t);
}
